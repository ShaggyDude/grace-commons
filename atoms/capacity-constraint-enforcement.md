...
- *Refining/rhetorical (open at the time; all closed 2026-08-25 — see the closure entry below):* C-F3 Invariants 5 and 13 unconditional where Invariant 4 is conditional; C-F4 [Now] card's `Parameter of` list (partially addressed 2026-08-24 — list completed to all seven consuming operations; the reviewer's objection to `Kind: Parameter` itself is declined, since Retention Window's [Now] card uses the identical kind); C-F5 undefined acronyms and the AI/PC/DP/TT/RW initialism cluster; C-F6 edge cases name composing patterns without links; C-F7 in-atom audit log's EOS boundary asserted not defended; C-F8 Intent's "one rule and one rule only" falsified by the atom's own surface; C-F9 Summary calls the log "permanent, attributed"; C-F10 no-op-adjust rejection is state-relative but assigned the field-format reason; C-F11 [Pool Id] has no primitive policy; C-F12 store-read failure has no named outcome; C-F13 [Available] both stored and derived; C-F14 `suspended`/`closed` rejection tokens have no cards.

**Final Critique 6 findings closure — 2026-08-25 (both foundational + twelve refining/rhetorical folded; gated by Final Critique 7 below).** Implementation pass closing every open Final Critique 6 finding. **Caller signatures are UNCHANGED**; the rejection vocabulary is unchanged (the one candidate new reason, a dedicated no-op-adjust token, was deliberately declined — [Invalid Request]'s scope is instead declared to cover degenerate no-effect requests, with the rationale stated). One line each, foundational first: C-F1 → every capability mis-pinned on Retention Window re-homed per §Capability provenance — the field-level attribution scrub is the deployment's declared shredding-class erasure mechanism under the composed retention *gate* (Invariant 8, check 3, and the Retention Window Composition note all restated; that atom declares only `place_under_retention` and `purge`), cold-storage tiering is a composed **Storage Tier** pattern *(forthcoming)* (Invariants 1 and 9), and the retention-boundary snapshot is the deployment's own archival-step artifact, produced before its purge runs (Generation acceptance's absolute-reconstruction mode) — Retention Window offers no snapshot surface; C-F2 → the injected [Pool Id]'s freshness pinned as a seam hard precondition with the runtime response named: the [Declare Pool] write is **create-only**, an observed collision refuses [Storage Failure] with a deployment alert, and the silent-overwrite reading is foreclosed (Decision points + the Id-generation edge case + Invariant 13's honest conditionality). Refining: C-F3 → Invariants 5 and 13 carry their conditions (Invariant 4's host obligations; the deployment's generator discipline); C-F4 → recorded as resolved-by-declination (the [Now] card's consuming-operations list was completed 2026-08-24; the objection to `Kind: Parameter` is declined on the Retention Window precedent); C-F5 → the check-8 initialism cluster (PC/DP/AI/TT/RW) spelled out and CPU glossed, and the same check's "retention manifests" corrected to Retention Window's actual retention records; C-F6 → edge-case pattern references converted to links (existing patterns) or **bold** *(forthcoming)* form (Storage Tier, Trusted Timestamping, Rate Limiter); C-F7 → the in-atom audit log's EOS boundary against Event Log defended in a dedicated edge case (the entries are the pool's arithmetic, atomic with the totals they snapshot — joint atomicity no composed stream supplies; Event Log/Audit Trail compose at deployment grain, keyed by this atom's event ids); C-F8 → Intent's "one rule and one rule only" restated as one load-bearing rule plus exactly the bookkeeping that keeps it checkable; C-F9 → the Summary's "permanent, attributed log" softened to append-only and attributed, with lifetime and attribution honestly bounded by composed retention; C-F10 → [Invalid Request]'s declared scope now covers state-relative degenerate requests, with the no-new-reason rationale; C-F11 → [Pool Id] given its primitive policy (byte-identity, no normalization, [Not Known] on miss); C-F12 → [Query] declared fail-stop with no [Storage Failure] arm (semantic rejection only; availability faults yield no partial snapshot); C-F13 → [Available] removed from the persisted record — derived-only, computed where reported, agreeing with its Terms card; C-F14 → the [Suspended]/[Closed] cards gain `Projects:` lines for their rejection-reason wire forms, stated as the state's own name projected. *Model impact:* the three `.tla` models untouched — the closures are provenance re-homing, prose, and validation-discipline statements outside model scope; harness re-verified green (correct passes; both buggy twins rejected); the formal-layer vote is unchanged.

**Final Critique 7 — 2026-08-25: not clean (1 foundational; ROUTED, not folded — closure round ends here).** The closing fresh-reader gate over the Final Critique 6 closure (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets and the spec body only; no Lineage, no prior findings). Both Final Critique 6 foundational closures held in their substance; the gate surfaced **one new foundational finding** plus five refining and one rhetorical — three of the refining being stale-attribution residue the C-F1 re-homing missed. Per the stop rule set before the round (one fix pass, one gate, then ground or route), all are recorded as **open routed findings**; the atom holds at `partially resolved` until they close and a round returns zero foundational. The human triager owns the sequencing. The foundational:

- *FC7-F1 — FIFO, SKU, DBA, PII unglossed — foundational (OPEN) →* four initialisms appear without first-use glosses, against the atom's own acronym discipline (the C-F5 closure glossed the check-8 cluster but not these). Fix: gloss each at first use.

*Refining/rhetorical (open, from Final Critique 7):* three phrases still attribute the boundary snapshot and scrubbing to Retention Window — "Retention Window's responsibility is the boundary snapshot", "supplied by the Retention Window boundary snapshot", and "Retention Window archive manifests and retention-boundary snapshots" — residue the C-F1 re-homing missed, with the Scrubbing-scope note and the Edge-cases→Retention passage carrying the same stale attribution; the in-atom audit log's self-witnessing claim overstated; the banking example's cents/dollars arithmetic incoherent; no length cap on caller-supplied actor refs. Rhetorical — the "a attributed" typo (introduced by the C-F9 rewording, fixed as an editorial touch in this same commit per [`pressure-testing.md`](../pressure-testing.md) §Editorial touches), a lowercase sentence start, and a lone-bullet formatting artifact.

**Final Critique 7 findings closure — 2026-08-26 (the one foundational + five refining + remaining rhetorical folded; gated by Final Critique 8 below).** Implementation pass closing every routed Final Critique 7 finding, per the re-grounding campaign (atoms first). **Caller signatures and the rejection vocabulary are UNCHANGED.** One line each, foundational first: FC7-F1 → the four initialisms glossed at first use — DBA (database administrator, Flow), FIFO (first-in, first-out, Behavior fairness note), SKU (stock-keeping unit, warehouse example), PII (personally identifiable information, the Edge-cases retention passage). Refining: the three stale-attribution phrases re-homed to match the C-F1 discipline — the Generation-acceptance preamble's given-records list now names Retention Window's retention records plus *the deployment archival step's* retention-boundary snapshots (and its "requires the deployment's Retention Window to produce boundary snapshots" became the archival step's obligation), the absolute-reconstruction mode's closing sentence now assigns the boundary snapshot to the deployment's archival step (with the "two records sets" typo fixed), and check 1's capacity-in-effect derivation now cites the deployment archival step's snapshot; the same pass corrected the two passages carrying the stale attribution — the Composition-note *Scrubbing scope* (the shredding-class erasure mechanism erases, gated by the composed Retention Window schedule) and the Edge-cases retention passage (scrubbing performed by the deployment's erasure mechanism under the retention gate; purging through Retention Window's own purge path); the per-event mode's "each event is internally self-witnessing" overstatement scoped honestly — the snapshot arithmetic is self-witnessing, the capacity-in-effect term comes from the surviving adjustment chain or boundary snapshot, so the mode is records-alone-within-window, not single-event-alone; the banking example's unit incoherence fixed by pinning the pool to cents throughout (capacity 1,000,000 for the $10k line; the declined purchase 1,200,000; the raised line 1,500,000) with the one-unit discipline stated and cross-referenced to *Resource semantics*; actor references given a deployment-pinned length cap in the Uniform validation rule (codepoint-counted like the reason cap; over-limit [Invalid Request]; existence-of-cap is the contract) with the Inputs bullet aligned. Rhetorical: the lowercase sentence start in *Clock semantics* capitalized; the lone-bullet formatting artifact on the in-atom-audit-log edge case flattened to the section's bold-heading paragraph form. *Model impact:* the three `.tla` models untouched — glosses, attribution re-homing, example arithmetic, and validation-discipline prose are outside model scope; the formal-layer vote is unchanged.

**Final Critique 8 — 2026-08-26: CLEAN (0 foundational); atom re-grounded.** The closing fresh-reader gate over the Final Critique 7 findings closure (AI-conducted, claude-fable-5, fresh-reader discipline throughout — pass question sets and the spec body only; no Lineage, no prior findings; an atom, so no constituent files). Every Final Critique 7 closure held. The round returned **zero foundational**, eight refining, and five rhetorical findings; per the campaign stop rule (one fix pass, one gate, ground on clean or route), the atom re-grounds at `grounded on Final Critique 8 — 2026-08-26`, and the residue is recorded here as open routed findings — non-blocking under the 92%-good threshold (foundational at zero). *Refining (routed, open):* the State transitions preamble's "each successful write appends one audit-log event" contradicts the [Declare Pool] row (four event classes are defined; declaration writes [Declared At] on the pool record, no event) — a literal reader would invent a fifth event class; Generation acceptance treats "[Declare Pool]" as a surviving/purgeable log record and never says where the replay's starting declared-capacity value is read from; Invariant 7's "never change" lacks the atom-view/composed-view qualifier Invariants 1, 8, and 9 carry, against the Scrubbing-scope note's permitted erasure of the declaration attribution fields; the post-Invariant-14 wrap-up miscites Invariants 8/9/10/11 for allocation-event completeness, a guarantee no numbered invariant states (it lives in Invariant 14's intended-records list and the transition table); the [Open] card carries no `Projects:` line though its value crosses the wire in [Query] `state` and [Prior State]/[New State], while [Suspended]/[Closed] both project; four composing homes named without link or *(forthcoming)* marker (Settlement Posting, Lease, Transaction, Notification Fanout); the HIPAA minimum-necessary citation pins §164.508 (authorizations) where §164.502(b)/§164.514(d) is the minimum-necessary home; I/O (and the flow example's DB) unglossed at first use against the atom's own acronym rule. *Rhetorical (routed, open):* the Summary/Outputs per-event verification phrasing is stronger than Generation acceptance's own bounding (adjustment chain needed, not single-event-alone); the [Now] card's `Parameter of:` line invites a signature-level misreading its own text forecloses; the [Storage Failure] card's "any action" sits loosely against [Query]'s declared fail-stop; Terms cards cite container types with no cards ("Pool", "the pool state", "the action rejection"); Behavior's "an upward adjustment is always accepted" omits the not-[Closed]/[Not Known] preconditions pinned elsewhere.
