...
- **[Party Identity](./party-identity.md)** — Party Identity is the persistent verifiable identity record of an external party. Invitation is the gate through which that party enters the system. External Onboarding wires them: an Invitation is accepted, supplying `accepting_identity_ref`, and a Party Identity record is created for that reference. Without Invitation, the library has no structured account of how an external party came to be in the system at all.
- **[Credential](./credential.md)** — in External Onboarding, credential registration follows acceptance. The `accepting_identity_ref` from the Invitation is the `principal_ref` passed to `Credential.register`. The Invitation record is the audit anchor that traces the credential back to the specific invitation event.
