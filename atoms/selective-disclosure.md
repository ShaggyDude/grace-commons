...
**Showcase pass — 2026-06-29.** Representational-only annotation/legibility pass; no guarantee, invariant, number, formula, signature, or rejection taxonomy changed. (a) **Four-kind `[Term]` annotation** applied across the body and a `## Terms` registry added before Composition notes (14 terms): 2 Operations ([Record], [Read]); 0 Types (the disclosure record is an ambient plain-noun referent — "Field of: the disclosure record" — mirroring notification); 8 Fields — 6 on the record ([Disclosure Id], [Subject Ref], [Recipient], [Scope], [Authority], [Disclosed At]) plus the two structured sub-fields [Authority Type] and [Authority Reference] ("Field of: the authority field", projecting `authority.type` / `authority.reference`); 0 Parameters (every business input is stored as-itself; the clock and id are pipeline-injected at the seam, not parameters); and 4 rejection Members ([Invalid Request], [Unknown Authority Type], [Storage Failure], [Invalid Query]). Survivors left backticked: the one labeled projected-contract signature per Operation; the `recorded(disclosure_id)` success return; the `authority.type` wire-enum values (`consent`/`legal-hold`/`regulatory`); the injected-seam tokens `now`/`clock_t`/`id_t` and the `not_future(...)` pure-guard predicate; the `store_name` deployment token; the non-existent `amend`/`retract`/`correct`/`delete` actions; concrete example calls, ids, values, and timestamps; and external standard tokens. The clock is injected at the seam and used only to default [Disclosed At] and to feed the not-in-future validation guard — no status is derived from it — so there is no `[Now]` term (mirrors legal-hold and notification). (b) **Summary/blockquote merge** — `## Summary` moved to the top (after TOC, before Intent), the descriptive top blockquote folded out after confirming each claim is carried by Summary/Intent/Identity model/Invariants 1 and 6; no *also-known-as* line existed, so none was invented. (c) **Lineage collapsed** into a `<details markdown="block">` block. (d) **prose cut #1** — the single-paragraph Summary split into one-idea-per-sentence paragraphs, lossless. (e) **prose cut #5 — skipped (with reason):** the atom has no state machine (a disclosure record has no lifecycle — it simply *is*); [Record]'s rejection ladder is already a linear priority chain stated in Decision points, and [Read] is a filter query — neither sharpens as a state or decision table. Re-verified, not re-grounded: Status stays at `grounded on Final Critique 5 — 2026-06-23`. Gates: lint clean (O-term resolver — every marker resolves and every card is used); term-adapter derives cleanly (14 terms); 6 invariants preserved; no formal model (formal-layer vote NO — English-only), so the harness gate is N/A.
