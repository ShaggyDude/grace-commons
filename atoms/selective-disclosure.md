...
- The current clock reading `now` (the pipeline's `clock_t`), injected at the atom's single I/O seam. Per the Logic Confinement Principle (see [`execution-contract.md`](../execution-contract.md)), the host reads the clock at the seam before the transition runs and supplies `now` as an explicit input to `record`; the transition reads no wall clock internally and `now` is not supplied by the business caller. It is consumed only to default `disclosed_at` when not supplied and to evaluate the pure not-in-future guard (see Decision points). `read` is a pure query over stored records and does not consume `now`.
...
- `record(subject_ref, recipient, scope, authority, disclosed_at?, now) → recorded(disclosure_id) | rejected(invalid-request | unknown-authority-type | storage-failure)` — create a new disclosure record. `now` is the **injected clock reading** (the pipeline's `clock_t`, supplied at the I/O seam — not read inside the transition, not trusted from the caller); it is used for exactly two purposes: (a) defaulting `disclosed_at` to `now` when the caller does not supply it, and (b) the pure not-in-future guard below. Assigns a fresh `disclosure_id`, records `subject_ref`, `recipient`, `scope`, `authority` (both `type` and `reference`), and `disclosed_at` (the injected `now` if not supplied; the resolved value must not be in the future). The record enters the store permanently; it is immutable and cannot be retracted. `subject_ref`, `recipient`, `scope`, and `authority.reference` must each contain at least one non-whitespace character — any violation is `invalid-request`. `authority` itself must be present and must carry both `type` and `reference` sub-fields — a missing or structurally malformed `authority` field is `invalid-request`. The resolved `disclosed_at` — caller-supplied or defaulted to the injected `now` — must not be in the future relative to that same injected `now` (`invalid-request`). `invalid-request` is checked before `unknown-authority-type`; structural and field-content validation completes before the authority type is evaluated. If all field-level preconditions pass, `authority.type` is checked: if it is not one of {`consent`, `legal-hold`, `regulatory`}, the call is rejected as `unknown-authority-type`. `storage-failure` if the store write fails after all preconditions pass; no `disclosure_id` is issued and no record enters the store. Rejection priority: `invalid-request` → `unknown-authority-type` → `storage-failure`.
...
2. **Calling system calls `record`.** Immediately following the disclosure, the calling system calls `record(subject_ref, recipient, scope, authority, ...)`. The atom assigns a `disclosure_id`, records all fields, and returns `recorded(disclosure_id)`.
...
**Logic confinement (clock and id).** The clock and the id are **injected inputs at the I/O seam**, never produced inside a transition. `now` (`clock_t`) is read once by the pipeline and passed to `record`; the `disclosure_id` is the injected `id_t` (system-generated at the seam, never reused). `now` is consumed for exactly two purposes: defaulting `disclosed_at` when the caller does not supply it, and the not-in-future guard. That guard is a **pure function of the resolved `disclosed_at` and the injected `now`** — `not_future(disclosed_at, now) ≜ disclosed_at ≤ now` — and it **rejects without writing** when it fails. This is genuinely execution-time *validation* (the residual that is *clearly marked*, not derived away): a disclosure cannot be recorded as having happened in the future, and that determination must be made against the clock at the moment of recording. No clock is read inside the transition. Rejection priority for `record`: `invalid-request` → `unknown-authority-type` → `storage-failure`.
...
`partially resolved` — touched 2026-06-21 by the **execution/render-time refactor** (reduce execution-time clock dependence; clearly mark the residual). This is the LIGHT transform with a NO formal-model vote: there is **no stored expiry to derive away** (a disclosure record simply *is*, with no lifecycle), so the refactor only (a) threads the injected clock `now` **explicitly** into `record(subject_ref, recipient, scope, authority, disclosed_at?, now)` — used to default `disclosed_at` and to evaluate the not-in-future guard — and (b) marks the not-in-future check as a **pure** guard over the resolved `disclosed_at` and the injected `now`, kept as the residual execution-time *validation* (clearly marked, not derived: a disclosure genuinely cannot be recorded as future-dated, and that is decided against the clock at recording time). A **Logic confinement (clock and id)** note was added to Decision points. A full three-pass re-pass (touch trigger) is **pending**; the next clean re-pass grounds at **Final Critique 5**. Prior grounding: `grounded on Final Critique 4 — 2026-05-20` (formal-layer vote stands NO — English-only). See Lineage §Execution/render-time refactor — 2026-06-21.
