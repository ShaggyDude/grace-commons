...
**Formal model — 2026-06-03: TLA+ authored and verified; pattern promoted to `grounded`.** Derived model [`legal-hold.tla`](./legal-hold.tla) + config [`legal-hold.cfg`](./legal-hold.cfg), checked by `tla-checker` via `tools/harness/check.mjs`. *What it checks:* up to `MaxH = 3` holds over one record, each {none, Active, Released}; the load-bearing **Invariant 4** (concurrent-hold independence — releasing one hold does not affect another) via a ghost `releasedByOwn[k]` (TRUE only if hold k was the release target): `Inv_HoldIndependence == ∀ k : holds[k] = Released ⇒ releasedByOwn[k]`. Exhaustive: 27 states, holds. *Buggy twin* [`legal-hold-buggy.tla`](./legal-hold-buggy.tla) cascades `release` to every Active hold (conflating per-hold release with record-level release); rejected at 12 states (place 2 holds, release one → the other is Released without its own action). *Out of model scope:* id discipline, attribution fields, the purge gate (the Defensible Retention composition — see `compositions/defensible-retention.tla`). *Conflict-protocol outcome:* none — the model **corroborates** the English; canonical English unchanged.

**Formal model extension — 2026-06-04: Invariant 6 GAP closed; `Inv_TemporalOrdering` added.** Extended [`legal-hold.tla`](./legal-hold.tla) with a global monotonically-advancing bounded clock `now` (a Naturals counter in `0..MaxClock`, advanced by a `Tick` action; no Sequences module — per harness README) and two per-hold ghost timestamp arrays: `placedAt[k]` (stamped with `now` on `Place`) and `releasedAt[k]` (stamped with `now` on `Release`). New invariant: `Inv_TemporalOrdering == ∀ k ∈ 1..MaxH : holds[k] = "Released" ⇒ releasedAt[k] ≥ placedAt[k]`. Because `now` only advances monotonically, the correct model holds exhaustively — release always stamps a tick ≥ the placement tick. `Inv_HoldIndependence` is retained and still checked in `Safety`. Config updated: `MaxH = 2`, `MaxClock = 3`; `Init` assigns all five variables. *State count:* 370 states (`MaxH=2`, `MaxClock=3`); raised to `MaxClock=4` → 811 states, invariants still hold. The state count grows with `MaxClock` because more absolute timestamp combinations exist — this is inherent to a timestamp-stamped model; no new behavioral patterns (ordering violations) emerge at higher bounds, confirming the temporal-ordering claim is fully exercised at `MaxClock=3`. *Buggy twin* [`legal-hold-buggy.tla`](./legal-hold-buggy.tla) updated: `Release` stamps `releasedAt[k] = 0` (backwards/skewed clock) instead of `now`; any hold placed at tick > 0 yields `releasedAt < placedAt`, violating `Inv_TemporalOrdering`. Rejected at 18 states (Tick → Place → Release sequence). The Inv 4 cascade-release hazard is retained as a **second isolated twin**, [`legal-hold-buggy-cascade.tla`](./legal-hold-buggy-cascade.tla) (cascade `release` flips every Active hold to Released but marks `releasedByOwn` only for the target; rejected at 25 states on `Inv_HoldIndependence`, with `Inv_TemporalOrdering` holding so the failure is isolated). The two isolated twins — `legal-hold-buggy.tla` (Inv 6) and `legal-hold-buggy-cascade.tla` (Inv 4) — give each load-bearing invariant its own reachable, checker-rejected counterexample rather than letting the shorter violation mask the longer; both are auto-discovered and required-to-reject by `tools/harness/audit.mjs`. *Conflict-protocol outcome:* none — the extension corroborates the English; `released_at ≥ placed_at` as enforced at the `release` Decision point is exactly what the model verifies.

**Showcase pass — 2026-06-29.** Representational-only annotation/legibility pass; no guarantee, invariant, number, formula, signature, or rejection taxonomy changed. (a) **Four-kind `[Term]` annotation** applied across the body and a `## Terms` registry added before Composition notes (23 terms): 3 Operations ([Place], [Release], [Read]); 11 Fields ([Hold Id], [Record Ref], [Placed By], [Hold Reason], [Placed At], [Case Ref], [State], [Released By], [Release Reason], [Released At], [Store Name] — the hold record carries no Type card, mirroring the plain-noun "hold record" referent); 2 Parameters ([Reason], [Query]); 7 Members ([Active], [Released] states; [Invalid Request], [Not Known], [Already Released], [Storage Failure], [Invalid Query] rejections). The one labeled projected-contract signature per Operation, the `release` success token `released`, concrete example invocations and their returns, the non-existent `release_all`, peer-atom tokens (`retention_until`, `purge`, `purged_at`, `purge_record`, `retained(legal-hold)`), model filenames, and dated registers stay backticked as survivors. Wall-clock timestamps are stamped from the receiving node's clock (this atom is not in the seam-injected-clock family — no `[Now]` term). (b) **Summary/blockquote merge** — `## Summary` moved to the top (after TOC, before Intent), the descriptive top blockquote folded out after confirming each claim is carried by Summary/Intent/State/Invariants; no *also-known-as* line existed, so none was invented. (c) **Lineage collapsed** into a `<details markdown="block">` block. (d) **prose cut #1** — the densest Summary run-on split into one-idea sentences, lossless. (e) **prose cut #5** — the State "Valid transitions" list rendered as a transition table (action / from / to / guard / stamps / result), with terminal-absorption and concurrent-hold-independence semantics kept in the prose beside it. Re-verified, not re-grounded: Status stays at `grounded on Final Critique 4 — 2026-05-20`. Gates: lint clean (O-term resolver — every marker resolves and every card is used); term-adapter derives cleanly (23 terms); 8 invariants preserved; `.tla`/`.cfg` untouched — harness re-run green: `legal-hold.tla` PASS, `legal-hold-buggy.tla --buggy` rejected (Inv 6), `legal-hold-buggy-cascade.tla --buggy` rejected (Inv 4).
