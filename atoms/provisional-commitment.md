...
- An injected wall-time input supplied at the seam, providing the current wall-time (clock time as a human would read it) timestamp `now` at each action invocation. Id material (the source of opaque, unique commitment identifiers) is likewise injected at the seam and not generated internally.
...
- `place_hold(resource, requester, duration)` → commitment enters Held with a fresh `id` (assigned from injected id material at the seam), `placed_at` stamped from the injected `now`, `expires_at = injected now + duration`. Returns `id`.
- `confirm(id)` → commitment moves Held → Confirmed; `confirmed_at` stamped from the injected `now`. Returns `ok`.
- `release(id)` → commitment moves Held → Released; `released_at` stamped from the injected `now`. Returns `ok`.
- `expire(id)` → commitment moves Held → Expired; `expired_at` stamped from the injected `now`. Returns `ok`.
...
- The commitment record persists in its terminal state indefinitely from the atom's perspective. Retention, archival, and purge (permanent, unrecoverable removal from storage) are composing concerns; the regulated-deployment composition is with [Retention Window](./retention-window.md).
...
- **`now` and the id material are injected inputs supplied at the deployment seam.** Every transition that reads the clock receives `now` as an explicit injected input, and `place_hold`'s id material is supplied by the deployment's source at the seam — per the Logic Confinement Principle (see `execution-contract.md`), the core transition neither reads a wall clock nor generates an id internally, so each transition is a pure function of its record state, inputs, `now`, and id material, with both sources remaining auditable at the deployment layer. The caller-facing action signatures (`place_hold(resource, requester, duration) → id`, `confirm(id)`, `release(id)`, `expire(id)`) are unchanged — `now` and the id material are not new caller parameters.
...
- **Invariant 10 — Commitment store durability.** Once recorded, a commitment is never deleted from the store. `confirm`, `release`, and `expire` transition a commitment to a terminal state; they do not remove the record. The total commitment count is monotonically non-decreasing. Retention, archival, and purge are composing concerns ([Retention Window](./retention-window.md)).
...
A merchant submits a $250 authorization against a customer's card. The bank places a hold (`id` = `auth_c41`, `placed_at` = now, `expires_at` = now + 7 days per scheme rules). The cardholder's available credit drops by $250. Three days later the merchant captures the authorization — `confirm(auth_c41)`; the $250 becomes a settled charge. Alternatively the merchant voids the authorization within seven days — `release(auth_c41)`; available credit restores. If the merchant does neither, the bank invokes `expire(auth_c41)` on the eighth day; available credit restores. Each transition is recorded for liquidity reporting under the bank's BCBS-aligned (Basel Committee on Banking Supervision — the international body that sets bank-capital and liquidity standards) framework.
...
- **Clock semantics.** Wall-time is accessed at the deployment seam — `now` is injected into each transition rather than read from an internal clock. Skew between the injected `now` and the underlying wall source, monotonicity, and timezone handling are deployment concerns (clock quality is a deployment-layer decision, not part of this atom's contract). The window's correctness is best-effort under an adversarial clock; a composed Event Log's `sequence_number` is the authoritative order when commitments race.
...
- **[Event Log](./event-log.md)** — for the audit-able commitment history. The container appends an event to a log instance on every successful action (`place_hold`, `confirm`, `release`, `expire`), preserving the full state-transition sequence for compliance. The commitment record remains the current-state projection; the Event Log is the journal from which the projection can be replayed and audited.
...
- **Identify the composing patterns active in this deployment.** Whether idempotency ([Duplicate Prevention](./duplicate-prevention.md)), full audit history ([Event Log](./event-log.md)), pool capacity (Capacity Constraint Enforcement), reversal of confirmed commitments (Reversal), and verifiable attribution ([Actor Identity](./actor-identity.md)) are wired in, and with what configuration.
