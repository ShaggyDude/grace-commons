...
- *Refining/rhetorical (open at the time; all closed 2026-08-25 — see the closure entry below):* P-F6 unreachable `invalid-ref` on Transform; P-F7 Generation acceptance check 5 contradicts Invariant 5 on [Recorded At]; P-F8 Invariant 10 unconditional against a composed purge; P-F9 undefined acronyms (DAG, SEC, DEA); P-F10 identity model self-contradiction on artifact-to-chain cardinality; P-F11 `archived`/`already-archived` split undefended and one token uncarded; P-F12 [Read] has no failure surface; P-F13 rejection-path examples miss three reasons; P-F14 breach scenario leans on best-effort [Recorded At]; P-F15 SEC 17a-4 satisfaction overstated.

**Final Critique 5 findings closure — 2026-08-25 (all five foundational + ten refining/rhetorical folded; gated by Final Critique 6 below).** Implementation pass closing every open Final Critique 5 finding. **Caller signatures are UNCHANGED** — the one candidate signature change (a custodian guard on [Transfer]) was deliberately declined: the Summary's overclaim was corrected instead, and the guard's absence is now *defended* as the designed asymmetry (the from-side is enforced by construction, a caller-supplied check would add nothing structural and would block receive-side recording; authorization is uniformly deferred). One line each, foundational first: P-F1 → Summary reworded to the three guarded actions and the by-construction transfer, with the asymmetry defended in the hand-to-hand Behavior bullet; P-F2 → an *Authorization* edge case added in Capacity Constraint Enforcement's shape (attribution ≠ authorization ≠ authenticity; Permissions in front of all five actions, Actor Identity for the credential binding); P-F3 → the per-append joint atomicity extended to all five appends in Transitions — every entry write commits atomically with its [Next Sequence Number] increment, with the two chain-field actions carrying their extra update in the same write; P-F4 → Invariant 5 renamed *Total order and density* and now states the gap-free `1..n` sequence check 4 verifies, with the mechanism (per-append atomicity; rejected actions consume no number); P-F5 → custodian comparison pinned to exact byte-identity with no normalization, in the [Current Custodian] field entry the guards cite. Refining: P-F6 → the rejection priority for [Transform]/[Disclose] reordered field-format-before-custodian-comparison, with the reachability rationale stated (an empty ref can never equal a non-empty [Current Custodian], so the old order made [Invalid Ref] unreachable there) and the Decision points aligned; P-F7 → check 5 verifies sequence position only ([Recorded At] later than the archive stamp is a clock artifact, not a finding); P-F8 → Invariant 10 scoped to this atom's own surface with composed retention disposal named out of scope; P-F9 → DAG and SEC glossed at first use (DEA's first use already carried its gloss); P-F10 → the identity model's cardinality corrected to one-chain-per-custody-episode, with the recurring-[Artifact Ref] cases named as exactly why [Chain Id] is the identity; P-F11 → the `archived` / [Already Archived] split defended at the [Archive] decision point (closed-to-you vs goal-already-achieved) with the card relationship stated; P-F12 → [Read] declared fail-stop with no [Storage Failure] arm — semantic rejections only, availability faults yield no partial sequence; P-F13 → three rejection-path examples added (`invalid-genesis-type`, `invalid-descriptor`, `already-archived`); P-F14 → the breach scenario brackets its wall-time window generously and confirms boundaries by sequence-adjacent replay; P-F15 → the SEC 17a-4 claims restated as necessary-not-sufficient in both the edge case and the Standards entry. *Model impact:* `provenance.als` / `provenance-buggy.als` untouched — the closures are prose, priority-order, and atomicity-obligation statements over the same backbone the model already checks (the density clause is true of the model by construction); the formal-layer vote is unchanged.

**Final Critique 6 — 2026-08-25: not clean (1 foundational; ROUTED, not folded — closure round ends here).** The closing fresh-reader gate over the Final Critique 5 closure (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets and the spec body only; no Lineage, no prior findings). All five Final Critique 5 foundational closures held; the gate surfaced **one new foundational finding** plus fourteen refining and two rhetorical. Per the stop rule set before the round (one fix pass, one gate, then ground or route), all are recorded as **open routed findings**; the atom holds at `partially resolved` until they close and a round returns zero foundational. The human triager owns the sequencing. The foundational:

- *FC6-F1 — erroneous-entry correction silently absent — foundational (OPEN) →* the append-only chain defines no path for correcting a wrong entry (a mis-keyed [Transformation Descriptor], a wrong [Recipient Ref]) and never declares correction out of scope — a fresh implementor cannot tell whether correction-by-append is intended, forbidden, or forgotten. Fix: an edge case either defining a correction-by-append discipline (a correcting entry citing the corrected [Sequence Number], the original immutable) or declaring correction an explicit non-goal with the composing home named.

*Refining/rhetorical (open, from Final Critique 6):* the breach-scenario continuity walkthrough misstates the sequence-adjacent replay rule; [Archive] lacks an [Invalid Ref] arm for a malformed chain ref; the concurrent-transfer edge case contradicts the serialized-execution contract; crash observability (what an observer may see of a joint atomic write interrupted mid-commit) unpinned; Generation-acceptance gaps, including no check witnessing Invariants 8 and 9; the [Originate] rejection-priority tie order unstated; the Transitions table miscites Invariant 9 where Invariants 3 and 5 are the ones at stake (an error introduced by the P-F3 closure edit); the `archived` Projects line and its card rationale conflict; the [Custodian Ref] card's all-entries claim disagrees with the transferred-entry field usage; no length caps on caller-supplied refs and descriptors; the artifact→chain lookup left unrouted; the Event Log paraphrase "consumes a sequence number" overstates the constituent's may-consume contract; DEA and RFC unglossed at first use; [Storage Failure] and [Invalid Query] carry no rejection examples. Rhetorical — the doubled edge-case headings ("Attempt to read a non-existent chain", "Disclosure scope and authority"; paste artifacts of the closure edit, fixed as an editorial touch in this same commit per [`pressure-testing.md`](../pressure-testing.md) §Editorial touches) and two overreaching phrasings.

**Final Critique 6 findings closure — 2026-08-26 (the one foundational + fourteen refining + two rhetorical folded; gated by Final Critique 7 below).** Implementation pass closing every routed Final Critique 6 finding, per the re-grounding campaign (atoms first). **Caller signatures carry no parameter changes**; the one contract change is additive — [Archive] gains an `invalid-ref` rejection arm — and it *resolves* rather than breaks a neighbor: Chain of Custody's routed refining finding that "validated by Provenance at each action" was overstated for archive is discharged by this arm (to be recorded there when that composition's closure round runs). One line each, foundational first: FC6-F1 → a *Correcting an erroneous entry* edge case added — correction-by-append is the defined discipline (a subsequent entry by the current custodian documenting the correction and naming the corrected [Sequence Number]; the erroneous entry stays immutable, error and correction both in the record), and formal amendment semantics (first-class amendment entries, supersedes links, an "effective" view) are an explicit non-goal routed to a composing Amendment pattern *(forthcoming)*. Refining: breach-scenario replay rule restated correctly (window completeness confirmed from an unfiltered read via the boundary entries' sequence-adjacent neighbors, with density (Invariant 5) closing the between-gap); [Archive] `invalid-ref` arm added to contract, decision point, and rejection priority, with the reachability note extended to [Archive]; the concurrent-transfer edge case rewritten to match the guard-free contract (both serialized transfers succeed; the second records a hand-off from the first's recipient; conditional transfer is a host/composition concern) — the old "should either succeed or fail" contradiction removed; the atomic-writes edge case rewritten as an all-or-none *observability* guarantee covering all five appends' coupled writes, with crash recovery required to complete or roll back dangling transitions before the store serves anything; Generation acceptance check 7 added witnessing Invariants 8 and 9 (event typing, genesis placement-by-type, id uniqueness), closing line updated to seven checks; [Originate] rejection-priority tie order pinned ([Invalid Ref] → [Invalid Genesis Type], refs in signature order); the Transitions miscite corrected (a lost counter update breaks Invariants 3 and 5, not 9 — an error introduced by the P-F3 closure edit); the [Archived] card now carries `Projects: archived` and states the shared-card projection the Decision points name; the [Custodian Ref] card and State bullet corrected — a `transferred` entry carries the [From Custodian Ref]/[To Custodian Ref] pair, not the single field; a *Length caps* edge case added (deployment-pinned caps on all six caller-supplied opaque strings; over-limit returns the field's format rejection, [Metadata]'s arm pinned to [Invalid Ref]) with the [Invalid Ref] and [Invalid Descriptor] cards aligned; a *Finding a chain from an [Artifact Ref]* edge case added (read surface keyed by [Chain Id] alone; reverse lookup is a host-side rebuildable derived index, deliberately not offered because [Artifact Ref] is opaque and non-unique); the Event Log paraphrase corrected to the constituent's may-consume contract ("a `storage-failure` may consume a sequence number"); DEA glossed at its Composition-notes first use and RFC at its Clock-semantics first use; [Invalid Query] and [Storage Failure] rejection-path examples added (inverted range; transfer during a store outage). Rhetorical: the round left the two overreaching phrasings unlocated, so the two strongest absolutes were selected and tempered honestly — the Summary's "structurally impossible to forge" (now: cannot be recorded through the action surface) and the disputed-transaction scenario's "makes a gap impossible" (now: admits no unrecorded holder) — both squaring with the tamper-evidence edge case's admission that a store-level adversary is out of scope. *Model impact:* `provenance.als` / `provenance-buggy.als` untouched — the closures are edge-case prose, rejection-vocabulary, card, and example changes over the same backbone the model checks; the formal-layer vote is unchanged.

**Final Critique 7 — 2026-08-26: CLEAN (0 foundational); atom re-grounded.** The closing fresh-reader gate over the Final Critique 6 findings closure (AI-conducted, claude-fable-5, fresh-reader discipline throughout — pass question sets and the spec body only; no Lineage, no prior findings; an atom, so no constituent files). Every Final Critique 6 closure held. The round returned **zero foundational**, ten refining, and six rhetorical findings; per the campaign stop rule (one fix pass, one gate, ground on clean or route), the atom re-grounds at `grounded on Final Critique 7 — 2026-08-26`, and the residue is recorded here as open routed findings — non-blocking under the 92%-good threshold (foundational at zero). *Refining (routed, open):* the [Transform]/[Disclose] projected-contract arm order disagrees with the rejection-priority order while [Archive]'s matches, inviting a wrong evaluation-order inference from contract order; the State section's cache-disagreement conformance claim miscites Generation-acceptance check 3, which replays the chain but never compares the replay against the stored cache field; Invariants 2 and 10 have no Generation-acceptance witness (a deleted newest non-archive entry passes every check unless check 6 happened to hold its id); the descriptor/recipient content rules are precondition-only — no invariant or check witnesses stored content the way Invariant 7 / check 1 witness custodians; no action exposes the chain-level [Artifact Ref] (entries do not carry it, no chain header is returned, and the gap is neither compensated nor declared a non-goal); the [Originate] priority is underdetermined for a call with both an over-limit [Metadata] and a bad [Genesis Type] (metadata rides the earlier [Invalid Ref] arm but sits after `genesis_type` in the signature order the tie-break invokes); RDF unglossed at first use (Standards, W3C PROV entry); Trusted Timestamping and the Transaction / Idempotent Reservation homes are named with neither a live link nor a *(forthcoming)* marker; the six [Event Type] wire values carry no Terms cards against the preamble's every-pinned-Member promise; the [Read] query surface leaves filter conjunctivity, range-bound inclusivity, and the time-range value format unpinned. *Rhetorical (routed, open):* the Summary's event enumeration omits `received`; Scenario 1's "all four conditions hold by Invariant 4" is citation-loose (conditions (a) and (d) are Invariants 3 and 8); the ISO 23081 "map directly" shorthand is unqualified beside the carefully-scoped SEC 17a-4 entry; the Identity model's "later chains for later episodes" narrative under-describes that simultaneous [Open] chains for one [Artifact Ref] are equally permitted; the Behavior bullet "no action other than [Transfer] changes [Current Custodian]" glosses [Originate]'s initial set; [Custodian Ref]/[Metadata] card-Kind and [Open]-card phrasing nits.
