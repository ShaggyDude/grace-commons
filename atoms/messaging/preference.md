...
- A principal reference identifying *who* the preferences belong to. Opaque — the principal registry is a separate concern. The atom requires only that principal references support equality testing (so `current_for` can locate the currently-in-effect record and the at-most-one-currently-in-effect rule can be enforced); it does not parse, normalize, or otherwise interpret their contents. No length cap is enforced by the atom; the deploying system is responsible for bounding `principal_ref` size to match whatever the underlying store and equality-check implementation can handle efficiently.
...
- **Confirm supersession atomicity at the timestamp level.** For any record that was superseded (a later record exists for the same principal with `set_at > this.set_at` and the gap is unaccounted for by explicit `delete` or `suspend`), `deleted_at` equals or is within the deployment's declared clock tolerance of the successor's `set_at`. The deployment must disclose its clock tolerance (the maximum gap expected between two writes within the same atomic operation, e.g., "writes within the same database transaction share a statement timestamp; tolerance is 0ms" or "wall-time at write time; tolerance is 500ms under NTP"). A gap materially exceeding the declared tolerance is a conformance failure: either supersession was not atomic, or a separate `delete` was issued in the gap and the operation timeline is not what the at-most-one rule would imply. The clock-tolerance disclosure belongs alongside the fanout-on-no-record policy disclosure required by check 6.
...
- **GDPR Article 21(2)** — right to object to processing for direct marketing purposes; objection must terminate marketing processing without delay. Full Article 21(2) compliance is a two-atom obligation: this atom records the delivery-preference signal of the objection (a per-channel opt-out value in `channel_preferences`), while the Consent atom must revoke the corresponding legal permission to process. A deployment that records an opt-out in Preference without also updating Consent has not fully discharged the Article 21 obligation. The Preference atom's contribution is the durable, immutable record of the principal's stated opt-out signal at the moment of objection; combined with the composing fanout pattern's enforcement and Consent's legal-permission revocation, the Article 21 obligation is satisfied.
...
`partially resolved` — foundation passes and one author-conducted refinement round complete in the initial drafting session; fresh-reader AI-conducted Phase 3 round complete (2026-05-25), all findings closed. Status will advance to `grounded` after the Opus Phase 4 Happy Torvalds X2 readiness check returns at-or-above the 95%-good threshold. Third entry in `atoms/messaging/`.
