...
3. **An event fires.** The composing pattern calls `subscribers_for(event_scope)` to enumerate all current subscribers for the event's scope, then creates a notification for each.
