...
- **External Onboarding** *(C16 — not started)* — the composition that wires Invitation acceptance to Party Identity creation, Credential registration, and Audit Trail attestation. The emergent invariant: a Party Identity reaches `Verified` only via an audit trail tracing back to a specific Invitation's `Accepted` transition; no Party Identity exists in the system without a named `inviter_ref` in the invitation record.
