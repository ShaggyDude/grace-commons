...
- [Provenance](./provenance.md) — append-only custody chain for a single artifact: origin, every change of hands, transformations, and disclosures, with a structural **custody-continuity** guarantee (exactly one current custodian at all times; transfers are hand-to-hand and cannot forge a predecessor, because the outgoing custodian is read from chain state, not supplied by the caller). The chain-of-custody primitive and the core atom of the forthcoming Chain of Custody composition (C12); the pharmaceutical and legal-evidence custody chains are the same structure. Anchors ISO 23081, W3C PROV, FDA 21 CFR Part 211, DEA 21 CFR Part 1304, SEC Rule 17a-4, and Federal Rules of Evidence 901(b)(9). `grounded` 2026-06-04 (Alloy model + buggy twin).
