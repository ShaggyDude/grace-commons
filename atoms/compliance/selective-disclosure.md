...
3. **Disclosure record persists indefinitely.** The record is in the store. It will appear in subsequent `read` queries. Retention governance — how long it must be kept — is a composing [Retention Window](./retention-window.md) concern. Tamper protection — ensuring it cannot be altered by a store administrator — is a composing [Tamper Evidence](./tamper-evidence.md) concern.
...
- **[Duplicate Prevention](../temporal/duplicate-prevention.md)** — for at-most-once semantics on `record` calls under retry conditions.
