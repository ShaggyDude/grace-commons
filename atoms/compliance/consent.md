...
- **At `check`** — `at_time`, if supplied, may be any timestamp — past, present, or future; `check` is a point-in-time query and accepts future `at_time` values (useful for "will this consent still be valid in 30 days?" pre-flight checks). When multiple records exist for (subject, purpose), the record with the latest `granted_at` is evaluated; when `granted_at` values tie, the record with the highest `consent_id` in lexicographic byte-order is used as a deterministic tiebreaker. The action never rejects; it returns one of four first-class outcome tags.
...
- **`check` evaluates the most recently granted record.** When multiple consent records exist for the same (subject, purpose), `check` uses the one with the latest `granted_at` among all records for that pair; when multiple records share the same `granted_at`, the one with the highest `consent_id` in ascending lexicographic byte-order is used as a deterministic tiebreaker. If that record is Revoked or Expired, `check` returns `revoked` or `expired` even if older Granted records exist — a new `grant` is required to restore a `granted` result. The full history is preserved; the gate check reflects the data subject's most recently expressed intent.
...
- **Invariant 6 — Expiry coherence.** A record with `expires_at` set must produce `expired` from `check` for any query where `at_time` ≥ `expires_at`. `expires_at` must be strictly in the future at grant time — a consent already expired at grant is meaningless and is rejected (`invalid-request`). The Expired stored-state transition must be written before the result of any `check` or `read` operation that evaluates the record past its `expires_at` is returned to the caller — whether that write occurs eagerly (background job at `expires_at`) or lazily (at first evaluation). An implementation that returns `granted` from `check` or `read` for a record with elapsed `expires_at`, without having written the Expired state transition, is non-conforming.
...
- **Expiry implementation.** The atom does not mandate whether Expired state transitions are written eagerly or lazily, but the write must occur atomically within the same operation that first evaluates the record past its `expires_at`, before the result is returned. An implementation that writes Expired eagerly (a background job at `expires_at`) is conforming; one that writes lazily must ensure the state write completes before the `check` or `read` response is returned to the caller. Either way, `check` must return `expired` for any query at or after `expires_at`, and `read` must reflect Expired state for any record past its `expires_at`. An implementation that returns a Granted record via `check` or `read` for a consent past its `expires_at` is non-conforming.
