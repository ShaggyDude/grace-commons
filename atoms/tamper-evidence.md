...
`grounded — 2026-05-20` — all required structural elements resolved; identity model explicit; action signatures with fully-named rejection taxonomies including `storage-failure` and `mechanism-verification-unavailable`; nine invariants including seal store durability (Invariant 9) and mechanism-soundness qualification on Invariant 2; five cross-domain examples plus `proof-invalid` and `record-set-mismatch` verification-failure examples; regulated adversarial scenarios; eleven edge cases. Third entry in `compliance`. Survived one foundation pass and one refinement round.
