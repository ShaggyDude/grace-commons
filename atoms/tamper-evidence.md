...
**Showcase pass — 2026-06-29.** Brought to the full showcase standard, matching the [`duplicate-prevention.md`](./duplicate-prevention.md) and [`actor-identity.md`](./actor-identity.md) exemplars (the latter shares this atom's shape — a single stable state plus a [Seal]/[Verify] pair with a multi-outcome [Verify]), on top of the already-applied annotation conversion. Changes are representational only: (1) **Summary/blockquote merge** — the plain Tier-1 [`prose.md`](../working-ideas/prose.md) cut-#4 Summary moved to the very top (immediately after the TOC, before Intent), and the descriptive top blockquote ("A compliance primitive…") folded out as redundant. Every load-bearing claim the blockquote carried was already present in the Summary, the Identity model, the State section, the Behavior bullets, or the invariants: cryptographic evidence the record set is unaltered since creation (Summary, Intent, Invariant 2); the opaque host-allocated immutable id with record-set-ref/proof/seal-timestamp/external-anchor as immutable properties (Identity model, State, Invariant 1); verification as a read-only query, not a state transition (Summary, State, Outputs); the requires-the-original-record-set asymmetry because the [Proof] commits to content (Summary, Behavior, Invariant 4); and the detectability contract (Summary, Intent, Invariant 2). So no claim needed folding in before deletion. No *also-known-as* line was added: this atom has never carried aliases, and none were invented. (2) **Lineage collapse** — the Lineage notes wrapped in the collapsed `<details>` block (with its `---` separator kept before it) mirroring the exemplars; no existing Lineage text altered. (3) **prose.md cut #1 (one idea per sentence)** — the Summary's single enormous sentence split into short declaratives; the seam-injection Inputs bullet (clock / id / cryptographic material) split into separate declaratives; the [Seal] and [Verify] Decision-points run-ons split, lossless. The §Leave it alone caveats were preserved: the tamper-evident-not-tamper-proof limit (Behavior, Edge cases), the verification-requires-records asymmetry, and Invariant 2's mechanism-soundness qualification all stay in force. (4) **prose.md cut #5 (prose→structure)** — the [Verify] outcome logic, which carries a real precedence order ([Not Known] first → [Record Set Mismatch] → [Mechanism Verification Unavailable] / [Proof Invalid] → [Verified]), rendered as a small precedence/decision table mirroring how [Actor Identity](./actor-identity.md#verify) rendered its [Verify] table, with the distinctions and read-only semantics kept in prose *beside* it per the cut-#5 caveat: the lookup-miss-resolved-first ordering; the [Original Record Set]-required asymmetry that routes an absent/wrong record set to [Record Set Mismatch], not [Not Known], because the [Proof] commits to content; the *why-each-reason-differs* distinctions ([Proof Invalid] = tampering detected, [Record Set Mismatch] = caller error / wrong records, [Mechanism Verification Unavailable] = transient/retryable, [Not Known] = id lookup miss); the three-first-class-outcomes framing; and the *writes-nothing on any path* read-only guarantee. The state machine is a single state ([Sealed]) with no transitions, so — exactly as [Actor Identity](./actor-identity.md) left its single [Attested](./actor-identity.md#attested) state as prose — it warrants no transition table and was left as prose; [Seal] was not also tabulated, as it adds nothing. Cuts #2 (glossary) and #3 (cross-ref footer) were assessed and **skipped**: acronyms are already spelled-out-once inline per the corpus convention here, and provenance already lives in the supporting prose and Composition notes rather than being re-cited mid-sentence. Expression only — every invariant and its number (1–9), Invariant 2's detectability claim with its mechanism-soundness qualification, both projected-contract signatures (`seal(record_set_ref, mechanism_credential) → …`, `verify(evidence_id, original_record_set) → …`), every guarantee, and the invariant count (nine) are unchanged in force; every `[Term]` marker still resolves to its card and the Terms registry is intact. The heading anchors inbound references rely on are unchanged. **Re-verified, not re-grounded:** Status stays at `grounded on Final Critique 4 — 2026-06-18`. Gates: linter 0 (incl. the O-term-resolver — all of this page's markers resolve against its registry); no formal model exists, so the harness gate is **N/A** (English-only, per the 2026-06-03 formal-layer NO vote); the derived manifest projects an identifier kind (Field) and an enumerated kind (Member); `git status` shows only this `.md` modified, no `.tla`/`.cfg`/`.als`; diff read line-by-line against the same-claim-or-weaker test.
