...
`close_pool` is terminal. Once closed, no new allocations and no capacity adjustments are admitted. Releases remain admissible so callers can unwind in-flight allocations cleanly — without this allowance, a closed pool would strand its running total at whatever value held at close time, and the per-allocation lifecycle in composing patterns (Provisional Commitment) could not reach its terminal states without a side channel. The pool record persists in Closed indefinitely; retention and archival are a composing concern.
...
**Invariant 3 — Closed is absorbing.** Once a pool enters Closed, no action transitions it elsewhere. `allocate`, `adjust_capacity`, `suspend_pool`, and `resume_pool` against a Closed pool are rejected. `release` is admitted in Closed (and only `release` is, among the mutating actions) so the running total can be cleanly unwound by composing patterns whose per-allocation lifecycles outlive the pool's policy lifecycle.
...
- **[Retention Window](../compliance/retention-window.md)** — for governing how long audit-log entries remain actively accessible. Invariants 8 and 9 make the log durable from the atom's perspective; Retention Window owns the policy for archival and eventual purge under regulatory schedules.
...
`partially resolved` — foundation draft complete; Pass 1 GRID structural completeness run with one finding closed in-pattern. Pass 2 EOS conceptual independence and Pass 3 Linus adversarial scrutiny pending; AI adversarial round and Phase 4 Opus clearance gate also pending. Authored 2026-05-14.

---

## Lineage notes

Capacity Constraint Enforcement is atom #8 in the ROADMAP's draft order and the third entry in `atoms/resource-lifecycle/` after Provisional Commitment and Soft Delete. The closest existing pattern is Provisional Commitment — same category, same kind of resource-encumbrance concern — and the draft mirrors its shape, identity-model discipline, and authoring conventions. The regulated-overlay sections (Regulated adversarial scenarios, Generation acceptance) are *not* included in this foundation draft: the atom is a utility primitive whose acceptance bar is not directly set by a regulator (no single standard owns capacity enforcement), and per the methodology's *When the conventions apply* clause in [`PRESSURE_TESTING.md`](../../PRESSURE_TESTING.md), the conventions are optional for non-regulated primitives. The atom's downstream compositions (Reservation Lifecycle and credit-limit-aware payment compositions) carry the regulated overlay at their layer; this atom's contract is regulator-agnostic. Pass 3 review may revisit this choice if the lack of adversarial scenarios proves to be a hidden-decision finding.
...
**One finding, closed in-pattern.** The initial draft was silent on the relationship between `close_pool` and `release`. Specifically, the action signatures in Inputs/Outputs listed `release(...) → release_event_id | rejected(not-known | over-release | invalid-request | storage-failure)` — no state-validity rejection at all — but it was not clear whether this was intentional (releases admitted in every state including Closed, for unwinding) or an oversight (Closed should reject releases the way it rejects other mutations). The transitions table said one thing implicitly (releases listed for "any state including Closed"); the action signature said another by omission. Resolved: the spec now states the design choice explicitly in three places — the action signature deliberately omits state-validity rejections; the Decision points entry for `release` names the policy ("`release` is permitted in all three states") with the rationale (closed pools still need clean unwinding of in-flight allocations); Invariant 3 (Closed is absorbing) clarifies that release is the only post-Closed mutating action permitted, and explains why (so the per-allocation lifecycle in composing patterns can reach its terminal state without a side channel). The behavior section also names this in the *close_pool is terminal* paragraph.

The reference graph re-walk after this fix is clean: Invariant 3 links to the State section's Closed-state description; the *Friction* surface (`closed` rejection from `allocate` and `adjust_capacity`) explicitly does not appear for `release`; Decision points names the asymmetry as a deliberate design choice.

**Pass 2 — Conceptual independence (EOS). Not yet run.** Foundation draft complete; Pass 2 will examine whether the audit-log machinery (allocation events, release events, adjustment events, state-change events) is correctly part of this atom or whether it should be extracted as a separate "Pool Audit Log" atom. The current draft scopes the log as a sub-record of the pool, by analogy with Party Identity's verification event list and state-change log; that choice is the EOS test for Pass 2.

**Pass 3 — Adversarial scrutiny (Linus mode). Not yet run.** Foundation draft complete; Pass 3 will probe the load-bearing decisions defended in-line — the three-state machine (vs. the roadmap's anticipated four), the aggregate-only identity model (vs. per-allocation), the release-admitted-in-Closed asymmetry, the preserve-by-precondition discipline on `adjust_capacity`, the choice to omit the regulated overlay — and will surface any happy-path bias in the five domain examples (none yet walk rejection-path examples at the example-narrative level, though Decision points enumerates the rejection paths exhaustively).
