...
**Scheduled rescan: 2026-05-20.** Pass 1 GRID — one foundational finding: *Regulated adversarial scenarios section absent despite required-when clause applying.* pressure-testing.md requires the section for "patterns elsewhere whose examples invoke regulated domains"; the Flow section and Examples explicitly invoke GDPR Article 17 erasure, and Standards references name GDPR, HIPAA, FRCP, and SOX. The prior Lineage notes assessed the section as "not required" but the methodology's required-when clause applies. Fixed: *Regulated adversarial scenarios* subsection added to Examples, walking regulator audit (GDPR supervisory authority verifying the erasure record), disputed erasure (data subject challenges purge status), and breach investigation (querying the Purged set by timestamp window for unexpected attribution). Pass 2 EOS clean. Pass 3 Linus clean. Round closes clean; status advances to 2026-05-20.

**Formal-layer vote — 2026-06-03: NO.** Linear state machine (Deleted→Active|Purged, no cycles); all invariants single-state structural, fully exercised by records-alone checks. Grounds English-only (minimum-formalism). Vote per [`pressure-testing.md`](../pressure-testing.md) §Formal models — The formal-layer vote.

**Showcase pass — 2026-06-29.** Representational-only annotation/legibility pass; no guarantee, invariant, number, formula, signature, or rejection taxonomy changed. (a) **Four-kind `[Term]` annotation** applied across the body and a `## Terms` registry added before Composition notes (26 terms): 4 Operations ([Soft Delete], [Restore], [Purge], [Read] — the [Soft Delete] card takes the `#soft-delete` anchor it shares with the page title, the same shape event-log's `[Event Log]` card ships with); 0 Types (the lifecycle record is an ambient plain-noun referent — "Field of: the lifecycle record" — mirroring notification); 11 Fields ([Record Id], [State], and the deletion/restore/purge attribution triples [Deleted By]/[Deleted At]/[Deletion Reason], [Restored By]/[Restored At]/[Restoration Reason], [Purged By]/[Purged At]/[Purge Reason]); 1 Parameter ([Reason] — consumed by [Soft Delete]/[Restore]/[Purge] and stored under the action-specific reason field, not under its own name); and 10 Members — the 3 lifecycle states ([Active], [Deleted], [Purged]) plus 7 rejections ([Invalid Request], [Already Deleted], [Already Purged], [Storage Failure], [Not Known], [Not Deleted], [Invalid Query]). Survivors left backticked: the one labeled projected-contract signature per Operation; the `deleted`/`restored`/`purged` success-outcome tokens; the non-existent `register` action; the `≥ deleted_at` / `purged_at ≥ deleted_at` order predicates and the `Deleted → Active` transition literals; concrete example calls, ids, values, and timestamps; the `store_name` deployment token; and external standard tokens. The wall clock stamps [Deleted At]/[Restored At]/[Purged At] and feeds a not-in-future / `≥ deleted_at` validation guard, but the [State] is set by explicit actions (not derived from the clock), so there is no `[Now]` term (mirrors legal-hold). (b) **Summary/blockquote merge** — `## Summary` moved to the top (after TOC, before Intent), the descriptive top blockquote folded out after confirming each claim is carried by Summary/Intent/State/Invariants 3 and 4; no *also-known-as* line existed, so none was invented. (c) **Lineage collapsed** into a `<details markdown="block">` block. (d) **prose cut #1** — the single-paragraph Summary split into one-idea-per-sentence paragraphs, lossless. (e) **prose cut #5** — the State "Valid transitions" list rendered as a transition table (action / from / guard / to / effect), with the fail-closed, terminal-[Purged], and two-step-purge semantics kept in the prose beside it. Re-verified, not re-grounded: Status stays at `grounded on Final Critique 4 — 2026-05-20`. Gates: lint clean (O-term resolver — every marker resolves and every card is used); term-adapter derives cleanly (26 terms); 8 invariants preserved; no formal model (formal-layer vote NO — English-only), so the harness gate is N/A.
