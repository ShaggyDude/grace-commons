...
- **[Capability-Backed Sharing](../compositions/capability-backed-sharing.md)** *(C15 — partially resolved)* — the composition that wires Capability redemption to Selective Disclosure, producing the bearer-token/regulated-audit worked example. The emergent invariant: the audit record reads "disclosed by bearer of capability X, allocated by actor Y" — allocator is identified, redeemer is structurally not.
