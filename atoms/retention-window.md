...
- [Place Under Retention] → retention enters [Retained] with a fresh [Retention Id], [Retained At] stamped from the injected [Now], [Retention Until] = [Retained At] + [Duration] and [Purge Deadline] = [Retention Until] + [Max Purge Delay] computed from injected scalars. Returns [Retention Id].
- [Purge] → permitted only while [Now] ≥ [Retention Until] (the pure eligibility guard; see Decision points); retention moves [Retained] → [Purged]; [Purged At] stamped from the same injected [Now]. Returns `ok`. (When [Now] < [Retention Until] the guard returns [Retention Period Not Elapsed] and writes nothing.)
