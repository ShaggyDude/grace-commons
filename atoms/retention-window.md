...
- `place_under_retention(record_ref, policy_ref, now)` → retention enters Retained with a fresh `retention_id`, `retained_at` stamped from the injected `now`, `retention_until = retained_at + policy.duration` and `purge_deadline = retention_until + policy.max_purge_delay` computed from injected scalars. Returns `retention_id`.
- `purge(retention_id, now)` → permitted only while `now ≥ retention_until` (the pure eligibility guard; see Decision points); retention moves Retained → Purged; `purged_at` stamped from the same injected `now`. Returns `ok`. (When `now < retention_until` the guard returns `retention-period-not-elapsed` and writes nothing.)
...
4. **Purge.** The host system invokes `purge(retention_id, now)`. The eligibility guard confirms `now ≥ retention_until`, the retention transitions Retained → Purged, and the underlying record is destroyed by the storage layer (or shredded cryptographically — see Composition notes).
...
- Wall-time is supplied as an injected input at the seam (threaded explicitly into `place_under_retention(..., now)` and `purge(..., now)`); the core transition reads no wall clock internally. Clock quality — honesty, monotonicity, skew relative to the regulatory clock — remains a deployment matter. Where retention deadlines have legal force (statute of limitations, regulatory clock), the implementation must source time from a trustworthy clock; a composed Trusted Timestamping pattern produces the verifiable time-anchor.

### Feedback

Each successful action produces an observable, measurable change:

- After `place_under_retention(record_ref, policy_ref, now)` — a new retention appears in Retained with a fresh `retention_id`, `retained_at`, `retention_until`, and `purge_deadline`. Retained count and total count each increase by one. The id is returned to the caller.
- After `purge(retention_id, now)` — the retention moves Retained → Purged with `purged_at`. Retained count decreases by one; Purged count increases by one; total count unchanged.
...
A bank places every settled transaction under retention with a 7-year policy (`policy_sox_settled_txn`: duration = 7 years, max_purge_delay = 30 days) via `place_under_retention(record_ref, policy_ref, now)`. At the seven-year mark, the records' `purge_eligible` projection flips to `true` (derived from the injected clock, not written); the bank's records-management system lists those records and invokes `purge(retention_id, now)` within the 30-day purge window. Each purge is logged for SOX (Sarbanes-Oxley Act — US financial reporting law) §802 audit. An external auditor querying *"any transaction record purged before its 7-year obligation?"* gets the empty set — Invariant 7 guarantees it.
...
```
place_under_retention(record_ref: "txn-1188", policy_ref: "policy-obsolete-v1", now: 2026-06-22T00:00:00Z)
...
`partially resolved` — touched 2026-06-21 by the **execution/render-time refactor** (derive purge-eligibility at read time; reduce execution-time clock dependence; clearly mark the residual). This is the LIGHT transform: there was no stored expiry flag to remove (Purged is a real write — it records that the purge happened — so it correctly stays stored, not derived). The injected clock `now` is now threaded **explicitly** into the `place_under_retention(..., now)` and `purge(..., now)` signatures; the `retention-period-not-elapsed` guard is marked a **pure** function of the stored record and the injected `now` (`state = Retained ∧ now ≥ retention_until`) and is kept, not weakened (it is the residual execution-time check, clearly marked, not derived away); and a derived read-time **`purge_eligible`** projection plus new **Invariant 11** (purge-eligibility is derived, never stored) make eligibility a computed condition rather than a stamped/stored flag. A Logic-confinement note was added to Decision points. A full three-pass re-pass (touch trigger) is **pending**; the next clean re-pass grounds at **Final Critique 5**. Prior grounding: `grounded on Final Critique 4 — 2026-06-18` (formal-layer vote stands NO — English-only). See Lineage §Execution/render-time refactor — 2026-06-21.
...
- *Sections updated for consistency:* summary already implicitly fine; Inputs (signatures + clock-purpose split), Outputs (new Read surface), State (transition signatures + guard note), Flow (purge step + projection), Decision points (Logic-confinement note + pure-guard marking), Behavior (purge-eligibility-derived bullet + explicit-thread note), Feedback (no-write-on-reaching-`retention_until` bullet + purge-eligible metric), Invariant 11 added, Examples (banking, both rejection-path examples, data-minimization scenario — all thread `now`), Edge-cases Clock semantics (explicit thread + two-readers-disagree note), Generation acceptance (new derived-eligibility check).
- *No formal model exists* (formal-layer vote is NO, English-only), so there is no model to re-derive; the records-alone basis the vote relies on is unchanged and is in fact reinforced (eligibility is now explicitly records-derivable).
- *Constituent-change cascade (Lineage-only note; composition files NOT edited):* threading `now` into the `place_under_retention` / `purge` signatures is a surface change to Retention Window. Compositions naming Retention Window as a direct constituent take a **touch-triggered re-pass** (confirming check, not done here): **Defensible Retention** (C1), **Audit Trail** (the canonical regulated-audit stack), **Chain of Custody**, **Immutable Transaction Ledger**, and **KYC / Customer Onboarding** (grep of `compositions/` for `retention-window.md`; transitively, Consent & Preference Management and Data Subject Rights Fulfillment reach Retention Window through those substrates).
