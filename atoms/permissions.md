...
- [Grant] → a new grant is recorded in [Active] with a fresh [Grant Id], the supplied [Subject Ref] and [Action Scope], and [Granted At] stamped from the injected [Now]. Returns [Grant Id].
- [Revoke] → the grant at [Grant Id] moves [Active] → [Revoked]; [Revoked At] stamped from the injected [Now]. Returns `ok`.
- [Check] → read-only query; no state change. Returns [Permitted] if any [Active] grant exists where `grant.subject_ref = subject_ref` and `grant.action_scope = action_scope`; otherwise [Denied].
