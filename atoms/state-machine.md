...
Workflow / State Machine is the general-purpose declared-state-machine primitive that Execute Gated Workflow and other multi-actor workflow compositions build on:
...
- **[Event Log](./event-log.md)** — the transition history within this atom has structural parallels to an Event Log (append-only, total-ordered by sequence number, `fired_at` best-effort), but Event Log has no notion of declared transitions, current state, or transition-validity enforcement. They are distinct freestanding atoms. The [Audit Trail](../compositions/audit-trail.md) composition (Event Log + Actor Identity + Retention Window + Tamper Evidence) can be layered alongside Workflow / State Machine instances to provide a tamper-evident, attributed, retention-governed audit substrate; that layering belongs to the Execute Gated Workflow composition.
