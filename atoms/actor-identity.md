...
- **[Credential](./credential.md)** — produces the credential the atom consumes. Credential answers *"did the right material arrive?"*; Actor Identity answers *"who authorized this action and can you prove it?"* The two atoms are distinct freestanding atoms sharing the same `principal_ref` / `actor_ref` namespace. Their relationship — cascade on revocation, permitted interchangeability of secret material, audit identity unification — is owned by the **[Authenticated Actor](../compositions/authenticated-actor.md)** composition *(partially resolved)*. See `demos/attributed-permissions-admin/CORNERS.md` §Cross-atom identity surface aliasing for the implementation-discovered gap that surfaces the need for that composition.
- **[Authenticated Actor](../compositions/authenticated-actor.md)** *(partially resolved)* — wires Credential and Actor Identity under a single principal, owning the three invariants neither atom currently specifies: revocation cascade, secret surface separation, and `principal_ref` / `actor_ref` namespace binding.
