...
- `atoms/` holds atoms, organized by category — `productivity/`, `temporal/`, `compliance/`, `resource-lifecycle/`, `messaging/`, `workflow/`, `healthcare/`.
...
│   ├── soft-delete              — Active → Deleted → Purged; Deleted is reversible
│   └── capacity-constraint-enforcement
│                                — named, bounded pool of a finite resource with total-allocated-never-exceeds-declared-capacity
├── compliance/
...
│   ├── legal-hold               — preservation obligation (a legal requirement to keep data) suspending purge (preventing deletion)
│   ├── consent                  — data subject agreement (a person's consent to how their data is used) for a named processing purpose (a specific, declared reason)
│   ├── selective-disclosure     — append-only disclosure accountability record: recipient, scope, authority
│   ├── party-identity           — persistent verifiable identity record for an external party with Unverified → Verified → Suspended → Closed lifecycle
│   ├── provenance               — append-only single-artifact custody chain with a hand-to-hand custody-continuity guarantee
│   ├── capability               — token-keyed authorization: a scoped, time-limited token grants access by possession, not identity
│   ├── credential               — secret-to-identity binding for an external party: register → verify | revoke
│   ├── invitation               — controlled-admission lifecycle: Issued → Accepted | Revoked | Expired
│   └── session                  — bounded-lifetime authentication attestation: Active → Expired | Revoked
├── messaging/
...
│   └── preference               — per-principal delivery-shaping record: channels, frequency, quiet hours, format
├── workflow/
│   ├── approval-step            — Pending → Approved | Rejected | Withdrawn; named-approver gate
│   └── workflow-state-machine   — general-purpose declared-state-machine: only-declared-transitions, one current state, replay-deterministic history
└── healthcare/
    ├── clinical-observation     — immutable clinical measurement with amendment/retraction trail
    └── medication-order         — prescription lifecycle from order through terminal resolution
...
├── external-onboarding          — Invitation + Credential + Party Identity + Audit Trail
│                                  ↳ emergent invariant:
│                                    invitation-gates-enrollment — no Party Identity is created
│                                    unless an accepted Invitation precedes it in the same
│                                    onboarding call, attested in one tamper-evident record
├── login                        — Credential + Session + Audit Trail
│                                  ↳ emergent invariants:
│                                    credential-gates-issuance, cascade completeness —
│                                    revoking a credential invalidates every session derived
│                                    from it in a single reconcilable operation
├── session-gated-authorization  — Session + Permissions
│                                  ↳ emergent invariant:
│                                    principal binding — the principal queried is always the
│                                    one extracted from the validated session, never a
│                                    caller-supplied value
├── privileged-access-provisioning — Multi-Party Approval + Credential + Session + Capability
│                                    + Audit Trail (substrate)
│                                  ↳ emergent guarantee:
│                                    approval-gates-allocation — elevated access exists only
│                                    after the full approval chain commits; the credential →
│                                    session → capability revocation cascade closes a
│                                    compromised principal's blast radius
...
│                                    Soft Delete's current-state summary discards
...
│                                    (revoke ⇔ complete-propagation-record)
├── kyc-customer-onboarding      — Party Identity + Retention Window + Audit Trail (substrate)
│                                  ↳ emergent guarantee:
│                                    verification-gates-activity — activity_permitted consults
│                                    the composition's own case index (verified-through-C8),
│                                    with adverse-trigger monitoring and a post-closure
│                                    retention floor that outlasts the relationship
├── reservation-lifecycle         — Capacity Constraint + Provisional Commitment +
│                                    Duplicate Prevention + Event Log + Actor Identity
│                                  ↳ emergent guarantee:
