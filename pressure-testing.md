...
**The composition-layer extraction question set — one level up.** The four questions above are written against *atoms*: they catch a concern that should be its own freestanding atom. As the composition catalog grows, the dominant over-absorption risk moves up a level. It is no longer a concern absorbed into one spec, but the *same emergent invariant or wiring decision recurring across many compositions* — binding bijections, cascade gates, attribution-coverage variants — with no canonical trigger for when that recurrence should be retired into a shared **substrate** composition or atom. The substrate pattern is the answer; without a stated trigger the decision gets made ad hoc, one round at a time, and the corpus accumulates parallel re-derivations of the same emergent rule. For a candidate recurring cross-composition invariant, ask:

- **Does it recur at the *invariant* level, not the atom-set level?** Count recurrence as *the same emergent invariant independently forced across compositions* — never as co-occurrence of the same constituent atoms. Similarity is not recurrence: shared constituent lists, parallel wiring shape, and naming conventions are surface similarity, while identical wiring can force different invariants and a genuinely cross-domain instance of one invariant can look nothing alike in two specs. Only invariant-level recurrence counts toward extraction; atom-set co-occurrence is the false signal that inflates the count.
- **Does the recurring wiring have its own invariant surface?** A substrate worth naming carries a must-be-true that no constituent owns alone — an emergent property in its own right, not the mere co-location of constituents. If the recurrence creates no new invariant — the compositions just happen to wire the same atoms — there is nothing to extract: it is a recipe, not a substrate, and naming it adds a constituent without adding a concept.
- **Would a substrate retire forthcoming-debt?** If the recurring invariant has already seeded forthcoming atoms or composition-introduced surfaces across the affected specs — each naming the same not-yet-extracted home — a substrate that collapses them into one declared owner retires that standing debt at once. Recurrence that retires forthcoming-debt already on the books is a stronger extraction signal than recurrence that would introduce a fresh dependency.
- **Does the extraction survive Jackson's wiring-not-primitive rule?** *An application is the wiring of freestanding concepts, not a new primitive.* Extract only when the recurrence names a real freestanding concept with its own invariant surface — not when it is wiring dressed up as a primitive. Wiring that recurs is still wiring; promoting it to a substrate makes every composition that names it inherit a dependency that buys no new guarantee. The first three questions can all read *yes* on surface evidence and this one still settles it: a real emergent invariant survives; a recurring recipe does not.

**Worked case — the decision that had no question set.** Immutable Transaction Ledger's Final Critique 5 (Pass 2) considered exactly this kind of extraction — a standalone *binding-bijection / dual-store atomic commit* atom — and rejected it, correctly leaving it as composition wiring under Jackson's rule. But it did so with no canonical question set to apply: the council reached the right answer by instinct, against that one spec, with nothing to make the next such decision repeatable or to keep a less careful round from extracting a recipe. The set above is that question set — so the next recurring emergent invariant is adjudicated against stated criteria rather than re-derived from scratch each time.
...
- **The grounding gate does not separate.** The formal-layer *vote* is mandatory for every pattern, vote-yes or vote-no — it is a grounding gate in the same pipeline as the three passes, and it carries the model-present bar by reference (a vote-yes pattern stays `grounded (English) — formal layer pending` until it clears the bar). Putting "the three passes plus grounding semantics" in one document and "the vote plus the bar" in another splits a single load-bearing rule across two files — the exact one-SSOT-per-rule violation the `spec-format.md` / Execution Contract ownership seam was assigned to *end*, reintroduced for a weaker reason.
- **The seam would be wide, not clean.** The formal layer interlocks with the process at points the split would have to cut: Pass 1's Proof node names the formal model; Pass 3's logic-confinement question is the model's determinism precondition; the coverage cross-check is a *scheduled-rescan sub-step*, not a freestanding formal concern; "what grounded means" is stated in terms of the vote and the bar. Each cut leaves a rule spanning the seam.
- **The corpus links section-into-file, and re-pointing is not free.** Most of the grounded corpus cites this document's §Formal models — The formal-layer vote, §Capability provenance, and the regulated conventions as file-level references that name the section in prose. A split re-points every one of them — and a *corrected cross-reference is itself a touch-trigger* (see §Touch triggers re-pass), so the mechanical fix would brush a `partially resolved` downgrade across the grounded corpus to relocate the weakest finding in the set. The cure costs more than the disease.
- **The change-history that looks extractable mostly is not.** The "canonicalized in Round N" and retro-mapping sentences read like changelog in canonical space, but most are *worked examples* discharging this document's own defend-each-claim-in-line discipline — the `capability.als` typecheck error justifying the model-present bar, Defensible Retention's relabel explaining the round-naming convention. Moving them to `changelog.md` would strip the in-line defense the methodology requires of every architectural claim.

The over-absorption is real but sub-threshold — preference-shading-to-finding — and the remedy's cost (a document seam to maintain, much of the grounded corpus touched, both halves owed a fresh adversarial read under this very section's recursive-application rule) exceeds the benefit. The condition that would re-open the decision is concrete: when the formal layer grows surface that *no* three-pass process step references — a second formal tool, or a model-authoring guide long enough to stand alone — it will have earned its own document, and the split becomes worth executing. Until then the single document holds.
