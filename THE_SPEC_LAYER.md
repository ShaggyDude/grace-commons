...
*Open question on the current axes.* The Grace Commons categories as of this writing — productivity, temporal, resource-lifecycle, compliance — deliberately mix conceptual axes: some are behavioral (temporal), some regulatory (compliance), some tier-of-difficulty (productivity primitives), some structural (resource-lifecycle). This is provisional. The right axes will be forced by content as the catalog grows past the size where preemptive cuts are reasonable; restructuring earlier would relocate the same confusion under different labels. Taxonomy refinement is deferred until the catalog's content drives the cuts, rather than the cuts driving the content. The discipline is to defer the structural decision until the data supports it — the same posture pressure-testing applies to atoms, one level up.
...
- **Alloy** (relational validity). Static structural checks and bounded LTL trace verification. Catches: malformed identity models, invariant violations over snapshots, structural attacks the prose didn't articulate (e.g., principal-binding exploits in Session-Gated Authorization). Tool: Alloy Analyzer / Alloy 6. Sibling files: `.als`. First natural targets: any composition with a rich relational structure — attribution maps, session-credential bindings, grant pairing maps.

- **TLA+** (temporal / concurrent validity). Exhaustive interleaving over every reachable state within bounded constants. Catches: race conditions, cascade-ordering violations, map-consistency failures under concurrent writes, TOCTOU races the action wiring must explicitly handle. Tool: TLC model checker. Sibling files: `.tla` + `.cfg`. First natural targets: any composition whose invariants involve ordering across actions — Login, External Onboarding, Attributed Permissions Admin, Session-Gated Authorization.
