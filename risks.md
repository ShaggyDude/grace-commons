...
- **Verification tooling (~40%) — working but unwired.** A mechanical linter (pre-alpha, proof-of-concept grade — six high-precision checks), a reproducible model harness, and a conformance validator with seven independent renders agreeing 20/20 all run today. None of it gates anything: no continuous-integration hook, no pre-commit hook — advisory-only.
