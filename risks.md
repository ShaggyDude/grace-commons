...
- **Verification tooling (~40%) — working but unwired.** A six-check mechanical linter, a reproducible model harness, and a conformance validator with seven independent renders agreeing 20/20 all run today. None of it gates anything: no continuous-integration hook, no pre-commit hook — advisory-only.
...
*Mitigation (in place):* One-SSOT-per-rule ownership seams (SSOT — single source of truth); the no-snapshot rule (dated records only); six built mechanical lint checks (dangling links, invariant counts, missing models, stale forthcoming-markers, count drift, invariant-number references). *(gap):* The built check classes lag the observed failure classes — status-mirror agreement, table-versus-list agreement, and duplicate rows are all mechanizable and all uncovered; nothing runs in continuous integration; contradiction sweeps are event-driven, not cadenced. The precise risk is **green-but-blind**: a passing linter read as corpus health while its checks don't cover what is actually failing.
...
*Mitigation (proposed):* Name the enforcement point — gate, cadence, or owner — in the same edit that lands the discipline; a discipline without one is recorded as *(designed, not deployed)*, never *(in place)*. *(gap):* A continuous-integration lint gate is one workflow file away and absent.
...
*Mitigation (in place):* The tracked-but-unpublished split (git-tracked **and** site-excluded) for working notes; the private directory is engine-only. *(gap):* The robust form is a private repository with its own version history; nothing blocks it. Also unresolved: the guided tool's design homes its work queues inside the same private directory the "engine-only" rule reserves — reconcile before either lands.
