...
11. **Stale-pin lint check** *(from the Mistral review pass, 2026-06-16 — adopt).* §4's cascade is a process; make it a CI gate. Add a `lint.py` check that flags any `local` extension pinned to a major line superseded by a MAJOR or marked deprecated — the version-pin analog of the existing `D-stale-forthcoming` check. This is *enforcement* of §4, not a new rule; it was the one concrete, adopt-on-sight item the external pass surfaced.
12. **Governance sign-off for a MAJOR on a universal spec** *(from the Mistral review pass — rerouted).* A breaking change to a *shared commons* atom or composition is high-stakes in a way an org-local one is not. §6 already requires a migration path; the open question is whether a universal MAJOR *also* needs an approval step — and if so it belongs in [`governance.md`](../governance.md), **not** as a review pass (the originally-proposed "Pass 4: Governance" collides head-on with the existing Phase 4 Opus clearance gate). Decide: does a universal MAJOR gate on governance approval, or is migration-path-plus-deprecation-cycle (§6) enough?

*Considered and declined (Mistral pass, 2026-06-16), recorded so a later reader does not re-raise them:* (a) "force `scope: local` on compositions, `scope: universal` on atoms" — a category error; Scope (commons-vs-org) and unit-type (atom-vs-composition) are orthogonal (§1), and the grounded compositions are `universal` commons citizens, not local. (b) "require deprecation timelines in Lineage" — mis-housed; time data lives in the changelog (§5), never in Lineage (the review arc), and §6 deliberately defers deprecation ceremony until there is external-consumer load.
