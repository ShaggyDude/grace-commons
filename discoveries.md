...
### 2026-08-26 — Who signs the record when the caller is gone, and who checked the caller was real: recovery attribution has an authentication seam under it

The joint recovery-machinery sweep (Multi-Party Approval and Execute Gated Workflow, closing both patterns' Final Critique 8 residues together) settled three questions the same way in both files: what a recovery record must carry (its own payload, including the human identity — never read off the event's attributed actor), which retried failures may actually be retried (transient only; the deterministic arms foreclosed by construction or given a terminal landing), and who signs a record emitted outside the invocation that constructed it (the composition actor, with the original human named in the payload and `recovery = true`). Both closures held under their gates. But Execute Gated Workflow's gate (its Lineage §Final Critique 9, F1) found what the third answer rests on: **composition-actor re-emission is a sound recovery move only if somebody verified the human's credential before the act committed** — and no composition in the corpus says who.

The trace is short. The composition takes a `credential` parameter and validates it as *parseable*. The constituent that authorizes the act — Approval Step's named-approver rule — is a byte-match on an actor reference and takes no credential at all. So the first and only credential verification in the whole flow is the substrate's attestation, inside the `record_action` that runs **after** the decision has committed. Before this sweep that ordering was merely uncomfortable: the deterministic `invalid-credential` arm had no landing, so a bad credential at least left a loud unresolvable alert. Giving it a landing removed the alert and let the act stand, attributed to the composition actor with an unverified human name in the payload. The fix made the failure quieter without making it safer — which is the general hazard in landing a deterministic arm: *a landing is only as good as the check that ran before the thing landed.*

The seam is corpus-wide, not local. Permissions' own spec assigns the actor-reference-to-authenticated-caller binding to "the composing system," and every composition that takes a `credential` and leans on a constituent's byte-match authorization *is* that composing system — and declares authentication neither in scope nor out. It is the largest unstated load-bearing assumption the campaign has surfaced: every records-alone attribution guarantee in the regulated patterns (electronic-signature attributability included) rests on it, and it is written down nowhere.

**Triage implication, recorded for the next round:** this is its own themed pass, and a wide one — sweep every pattern taking a `credential` parameter and state the obligation explicitly, in one of the two honest forms: *the credential is verified against the actor's registry material before any constituent call* (in scope, wired, with a pre-commit caller-fault arm), or *an authenticated-caller binding is a named deployment prerequisite this composition assumes* (out of scope, declared, and the audit surface inherits the assumption). Either is defensible; silence is not. And the recovery landing then conditions on it: a credential that fails verification pre-commit is a caller fault refused before anything commits, and composition-actor re-emission covers only the credential that was valid at the gate and was revoked mid-flight — the case it was actually designed for.
