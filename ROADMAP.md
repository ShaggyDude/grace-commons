...
Of 38 grounded patterns, 14 are fully `grounded` and 24 are `grounded (English) — formal layer pending` following the 2026-06-03 formal-layer vote sweep (aggressive bar) and the first backlog model landing. KYC / Customer Onboarding (C8) grounded on Final Critique 4 (2026-06-03). **Formal-model backlog progress (2026-06-03):** Party Identity is the first backlog model landed — `party-identity.tla` + buggy twin, exhaustive 532 states clean, verified via `tools/harness/`; see its Lineage §Formal model. The reproducible dual harness (`tools/harness/`: TLA+ via `tla-checker` WASM, Alloy via the `alloy.dist` jar under an npm-provisioned JRE 17) was stood up and all pre-existing models audited through it — surfacing a never-typechecked assertion in `capability.als` (fixed) and two further findings (see §"Harness audit findings" below). The next move on the atom side is one of the two remaining unstarted planned atoms (#7 Provenance, #9 Workflow / State Machine); on the composition side, one of the nine unblocked-and-unstarted compositions (C2, C3, C6, C7, C9, C11, C15, C17, C18 — see compositions section).

**Formal-layer vote sweep — 2026-06-03.** A formal-layer vote was cast for all 38 grounded patterns per `PRESSURE_TESTING.md §Formal models`. The 13 that remain fully `grounded`: model present (7) — Capability, Attributed Permissions Admin, External Onboarding, Login, Privileged Access Provisioning, Session-Gated Authorization, Multi-Party Approval; voted formal-not-warranted (6) — Actor Identity, Selective Disclosure, Tamper Evidence, Personal Todo, Soft Delete, Notification Fanout. The other 25 voted YES but have no formal model authored yet; they are downgraded to `grounded (English) — formal layer pending` and constitute the model backlog. **Update 2026-06-03:** Party Identity (the first of the 25) has since landed its model and returned to fully `grounded`, leaving 24 pending.
