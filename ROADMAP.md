...
| Provisional Commitment | TLA+ | Med (clock) | confirm-within-window (expiry race) — **kept YES** (genuine confirm-vs-auto-expiry race) |
| Duplicate Prevention | TLA+ | Med (clock) | single-recording; window monotonicity over advancing time — **kept YES** (concurrent first-record race) |
...
Tally (original aggressive bar): ~4 Alloy/Sonnet, ~21 TLA+/Opus. **After the 2026-06-03 reconsideration and the first eight landings**, the TLA+ queue is: 8 landed, 3 downgraded to English-only (Retention Window, Session, Consent), leaving ~10 TLA+ models pending (including the two kept candidates, Provisional Commitment and Duplicate Prevention) plus the 4 Alloy/Sonnet structural drafts. The bar was the right lever, exactly as anticipated here: the clock-based and precedence entries were the defensible "English + records-alone is sufficient" reconsiderations, and three of the five cleared it. Harness is settled (`tools/harness/`), so each model's pass/fail is mechanical.
