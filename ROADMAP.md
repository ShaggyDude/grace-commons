...
- `atoms/compliance/` (13): Actor Identity, Capability, Consent, Credential, Invitation, Legal Hold, Party Identity, Permissions, Provenance, Retention Window, Selective Disclosure, Session, Tamper Evidence
- `atoms/healthcare/` (2): Clinical Observation, Medication Order
- `atoms/messaging/` (3): Notification, Preference / Personalization, Subscription
...
**Category:** `atoms/compliance/` — resolved to compliance (compliance-infrastructure primitive, regulated overlay).
...
Compositions are sequenced by readiness. Of the seventeen C-numbered compositions, **nine are grounded** (C1, C4, C5, C8, **C10**, **C12**, C13, C14, C16); eight are unblocked and not started (C2, C3, C6, C7, C9, C11, C15, C17 — plus C18); none is blocked on a remaining atom. **C10 (Stateful Workflow Execution) and C12 (Chain of Custody) both grounded 2026-06-04** — C10 immediately after its spine atom Workflow / State Machine, C12 immediately after its core atom Provenance. C11 became unblocked when Preference / Personalization grounded on 2026-05-29. Provenance also enriches three other compositions (C6, C7, C8) as an optional composing atom for chain-of-custody guarantees — those compositions remain unblocked without it, but gain emergent invariants when composed with it once it lands.
...
The `workflow/` category now holds two atoms (Approval Step and Workflow / State Machine, the latter `grounded` 2026-06-04). The one-atom-category concern is **resolved**: the category stands on the fixed-state pole (Approval Step — states fixed by the atom) and the general-declared pole (Workflow / State Machine — states declared by the deployment) of the workflow primitive. The broader axial-split question across all categories remains open (see next paragraph).
