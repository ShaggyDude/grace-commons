...
**Conventions inherited from prior work.** Drafted with the *Regulated adversarial scenarios* examples subsection and *Generation acceptance* standalone section baked in from the first draft, following the convention Provisional Commitment introduced and Actor Identity carried forward. The composition pattern (wrap each underlying action with a token-cache check, cache every outcome, preserve constituent invariants) follows the structural template Undo History established for applications.
