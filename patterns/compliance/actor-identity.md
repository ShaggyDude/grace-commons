...
- Actions:
  - `attest(action_ref, actor_ref, credential) → attestation_id | rejected(reason)`
  - `verify(attestation_id) → verified | failed-verification(reason) | not-known`
...
- **At `attest(action_ref, actor_ref, credential)`** — `action_ref`, `actor_ref`, and `credential` must be well-formed; otherwise `invalid-request`. The credential must validate against the actor registry's public material for `actor_ref`; otherwise `invalid-credential`. The credential is *consumed*, never stored.
- **At `verify(attestation_id)`** — `attestation_id` must reference a recorded attestation; otherwise `not-known`. (This is a lookup miss, distinct from a verification failure.) The stored `proof` must check out against the stored `action_ref` and `actor_ref` using the actor registry's current public material; otherwise `failed-verification(proof-invalid)`. The verify call itself never *rejects* in the same sense as other action calls — `verified`, `failed-verification`, and `not-known` are the three legitimate outcomes, and a composing pattern should treat each distinctly.
...
Each rejected `attest` action produces an observable refusal: `invalid-request` or `invalid-credential`. Each `verify` outcome is a first-class result (not a rejection): `verified`, `failed-verification(proof-invalid)`, or `not-known`.
...
- **Invariant 6 — Self-containment.** `verify(attestation_id)` requires only the attestation's stored fields and the actor registry's public material for `actor_ref`. No additional out-of-band data is consulted at verify time.
...
The canonical regulated-audit stack composes Event Log + Actor Identity + Retention Window + Tamper Evidence as four freestanding atoms; the **Audit Trail** application *(forthcoming, in `applications/`)* is the wiring.
...
**Conventions inherited from Provisional Commitment.** This atom is the second regulated atom in the library and was drafted with the two new conventions Provisional Commitment introduced — *Regulated adversarial scenarios* as an examples subsection and *Generation acceptance* as a standalone section — baked in from the first draft rather than added in a second iteration. The atom is also the first to resolve a forthcoming-link that another published atom (Provisional Commitment) named; the existence of Actor Identity converts Provisional Commitment's `(forthcoming)` markers into resolved cross-references.
