...
- Actions:
  - `place_under_retention(record_ref, policy_ref) → retention_id | rejected(reason)`
  - `purge(retention_id) → ok | rejected(reason)`
...
- **At `place_under_retention(record_ref, policy_ref)`** — `record_ref` and `policy_ref` must be well-formed (rejection reason `invalid-request`). The policy's `duration` must be positive and `max_purge_delay` must be non-negative; otherwise `invalid-policy`. The host system is responsible for ensuring the record_ref refers to an existing record; the atom does not validate against the host's record store.
- **At `purge(retention_id)`** — `retention_id` must reference a retention currently in Retained; otherwise `not-retained` (already Purged) or `not-known` (no such id). `now ≥ retention_until` must hold; otherwise `retention-period-not-elapsed`. The atom does *not* reject purges that arrive past `purge_deadline` — at that point the regulator expects purge to have happened, and refusing the late purge would compound the overshoot. The lateness is observable in `purged_at` relative to `purge_deadline`; the audit names it.
...
Each rejected action produces an observable refusal: `invalid-request`, `invalid-policy`, `not-retained`, `not-known`, `retention-period-not-elapsed`.
...
- **Invariant 8 — Purge timestamp consistency.** For any Purged retention, `retention_until ≤ purged_at`. The atom does not constrain `purged_at` relative to `purge_deadline` — overshoot is observable but not forbidden.
...
The canonical regulated-audit stack composes Event Log + Actor Identity + Retention Window + Tamper Evidence as four freestanding atoms; the **Audit Trail** application *(forthcoming, in `applications/`)* is the wiring. With this atom landed, three of the four constituents are now grounded.
