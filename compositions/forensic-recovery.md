...
formal: verified — forensic-recovery.tla + 1 twin, 2026-06-11
last gate: 2026-08-26 — authentication-precedence gate, fresh reader — 4 foundational routed (1 since closed), 1 foundational and 6 refining corrected in-round, 13 refining and 3 rhetorical open

open:
- 2026-08-26-a · foundational · Composition state; [Recover History] step 4; check 3 · Composition state says reads rebuild on miss; step 4 treats an absent entry as empty and reports a binding gap, so a crash between the outcome write and the index append returns `history-incomplete(binding-gap)` forever from records that are all present, and check 3 compares the output against the index it was built from → make step 4 rebuild on miss; give check 3 a test that does not close the loop on its own input
- 2026-08-26-b · foundational · [Recover History] ordering; Invariant 3; serialization obligation · a delete whose outcome write fails, a restore, then the delete's compensation landing yields a replay in which the record was restored before it was deleted, returned `history-complete`; serialization scopes the critical section to the transition, not the action, so interleaved audit writes commit out of order; no verdict class exists for out-of-order → order by an occurrence-time key the compensation preserves, widen serialization to the whole action, add the verdict class
- 2026-08-26-c · foundational · [Recover History], `verify_record` input; [Attestation Verification] · one payload per event is passed where `interval-based` and `on-demand` cadences require the covering seal's whole range, so every verification under those cadences returns `seal-record-set-mismatch`, indistinguishable from tampering → surface the covering range through `read_record`, or declare `per-event` cadence a wiring requirement
- 2026-08-26-d · refining · every taxonomy quotation · the substrate's `recording-failure(step)` payload is dropped → carry the `(step)`
...
- 2026-08-26-f · refining · [Attestation Verification]; [Overall Verdict] · omits `attestation-registry-unavailable`, `seal-mechanism-verification-unavailable`, `partially-purged-coverage` and the `(compensation-window)` qualifier, none with a cell in the failure classes; `partially-purged-coverage` is reachable through the retention-asymmetry case → enumerate and place them
- 2026-08-26-g · refining · Generation acceptance check 2 · verifies `data.actor_ref`, a field the wiring never writes → verify what is written
...
- 2026-08-26-n · refining · substrate reads · two reads reach around the declared surfaces (event metadata without naming `read_record`; the substrate's internal `event_to_retention` directly) → route through declared surfaces
