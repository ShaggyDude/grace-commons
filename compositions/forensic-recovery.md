...
`grounded — formal coverage of Invariant 4 pending` (English grounded 2026-06-04; the TLA+ model [`forensic-recovery.tla`](./forensic-recovery.tla) + buggy twin verified in `tools/harness/` covers Invariant 4's atomic-commit arm, but its compensated arm is an open coverage GAP — finding MC-C3-1, with Invariant 2's compensated-path coverage riding it; see the coverage matrix and Lineage §Formal model). Sonnet-drafted against an Opus plan, then Opus-gated through Pass 1 (GRID), Pass 2 (EOS — the purge-eligibility-gate boundary against Defensible Retention (C1) holds; no over-absorption), Pass 3 (Linus), and a Final Critique round: one foundational finding, closed in-pattern (see Lineage notes). Regulated-pattern conventions (Regulated adversarial scenarios; Generation acceptance two-subsection split) baked in from the first draft. The formal-layer vote was YES; the derived TLA+ model — binding-bijection / no-dangling-partial atomicity ("no purge without an audit record"), mirroring `chain-of-custody.tla` — verifies green (4 states, all invariants hold) with a buggy twin the checker rejects (a non-atomic split leaving a purge with no audit event). The English cleared the 92%-good threshold (foundational findings at zero). The formal layer verifies Invariant 4's atomic-commit arm but not its compensated arm — the *Cross-store consistency under partial failure* path the spec mandates, on which Invariant 2's purge-accountability coverage also rides — which a later coverage cross-check surfaced as an uncovered GAP (finding MC-C3-1, recorded in [`../tools/harness/coverage/forensic-recovery.md`](../tools/harness/coverage/forensic-recovery.md)); the status carries `formal coverage of Invariant 4 pending` until the model is extended to cover it, a fix that rides C3's own touch-triggered round.
