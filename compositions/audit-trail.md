...
- **`record_action(action_ref, actor_ref, credential, data) → event_id | rejected(reason)`**
...
- **`seal_now() → evidence_id | rejected(reason)`** — under interval or on-demand cadence, seals the current unsealed tail. The presented record set is the slice `[sealed_through + 1 .. tail]`; the call delegates to `TamperEvidence.seal(slice_ref, mechanism_credential)`, records `seal_coverage[evidence_id]` and advances `sealed_through`.
...
- **`purge_eligible() → list of event_ids`** and **`purge_event(event_id) → ok | rejected(reason)`** — for any event whose retention has elapsed (`now ≥ retention_until`), the application cascades:
...
- **Invariant 3 — Integrity coverage (modulo unsealed tail).** Every event in the audit log with `sequence_number ≤ sealed_through` is covered by exactly one seal in the seal store. The unsealed tail is observable as a bounded gap; tighter `seal_cadence` shrinks it.
- **Invariant 4 — Cascade-on-purge.** When an event is purged, its corresponding attestation is purged in the same cascade step (per its meta-retention policy) and its `seal_coverage` entry is marked records-purged. No retained event is left without attestation or integrity coverage; no purged event leaves dangling attestation or seal-content claims.
...
`grounded` — composition logic specified across all four constituent atoms; emergent state (`event_to_attestation`, `event_to_retention`, `seal_coverage`, `sealed_through`) named; action wiring covers record, seal, verify, and cascading purge; eight application-level invariants stated and justified; walkthrough plus five cross-domain examples (banking SOX, healthcare HIPAA + 21 CFR Part 11, payments PCI DSS, pharmaceutical 21 CFR Part 11, broker-dealer SEC Rule 17a-4) and three adversarial scenarios; edge cases enumerate the deployment-shaped concerns (multi-instance, legacy events, cadence trade-off, credential compromise, policy reconciliation, right-to-erasure, legal hold, storage tier, durability, cross-store consistency, unsealed-tail policy); Generation acceptance bar explicit. Third entry in `compositions/`. The first application composing four atoms — and the canonical regulated-audit primitive the library has been forecasting since the first regulated atom landed.
