...
**Composition-state classification fold — 2026-08-26 (batched pre-convention tail, methodology debt #9; gated by Final Critique 5 below).** Convention retrofit for the composition-state rule adjudicated 2026-06-10, which postdates this composition's grounding. Change: the derived Personal-Todo-shaped state classified **derived index by construction** — the Event Log is the sole truth per the load-bearing wiring decision, §Replay semantics is the named rebuild procedure, the three Contract obligations hold trivially (no second truth-bearing write; lost materialization = rebuild trigger; no consistency claim beyond replay determinism, Invariant 4), and a materializing implementation's cache is declared an ordinary derived index that never substitutes for a fresh replay in invariant evaluation. The classification names what the design already was — this composition is the corpus's canonical derived idealization (composition-state audit row 41). **Caller signatures UNCHANGED**; no invariant number, signature, or rejection taxonomy changed. Gate: closed by Final Critique 5 below.

**Final Critique 5 — 2026-08-26: not clean (2 foundational; ROUTED, not folded — the tail round ends here).** The closing fresh-reader gate over the 2026-08-26 convention fold (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). Pass 2 returned clean — the derived-index-by-construction classification confirmed, no over-absorption, and the gate's hardest re-litigation (do replay preconditions survive arbitrary undos?) found the design sound: LIFO undo means a non-undone event's predecessors never change, so every replayed event revalidates against exactly the state it was validated against. It surfaced **two foundational findings** plus six refining and three rhetorical. Per the stop rule (one fold, one gate, ground or route), all are recorded as **open routed findings**; the composition is downgraded to `partially resolved` until they close and a round returns zero foundational. The two foundational:

- *FC5-F1 — the no-op [Edit] is an undefined-behavior hole — foundational (OPEN) →* Personal Todo declares the normalized-equal edit an accepted no-op that never writes (no `last_edited_at` change, no `storage-failure` possible); the composition's wiring appends on every accepted edit — either the append happens (violating the constituent's no-op contract, and making the no-op an [Undo] target that visibly changes nothing) or it is skipped (violating Invariant 1's every-successful-action-appends-exactly-one-event as written). Fix: mirror the constituent — a no-op [Edit] appends nothing, returns `ok`, cannot `storage-failure`, is not an [Undo] target — and carve Invariant 1 to *state-changing* actions.
- *FC5-F2 — Invariant 7 (reachability of prior states) is falsifiable as stated — foundational (OPEN) →* add → complete → undo leaves the Done state a prior composition-visible state that no sequence of undos can revisit (reaching it is Redo, declared out of scope); the invariant's carve-out covers forward-actions-after-undo but not the state each undo itself abandons. Fix: restate as walking back through the states of the current non-undone sequence, with undo-abandoned states explicitly Redo's territory.

*Refining/rhetorical (open, from Final Critique 5):* Invariant 4's all-constituent-invariants-hold overstated for Personal Todo's temporal invariants (deliberately rewound on the exposed timeline — propagate the per-replay scoping the [Undo] wiring already states); the delete `snapshot` and edit `prior_description` fields are never consumed by replay while the schema claims them replay-sufficient (drop or re-justify, and correct the [Snapshot] card); Event Log's `invalid-payload` arm unmapped and not foreclosed; the four forward actions' "update the derived state" lacking the Replay-semantics citation [Undo] carries; GoF, API, and UX unglossed; the invariants section never names its verification surface (one sentence declaring records-alone replay checks). Rhetorical — the `event_id`/`recorded_at` supply mis-cited to the immutability invariant (supply is the seam's; immutability is post-append); the [Nothing To Undo] card's vacuous empty-log coverage; Invariant 6's restored-field list omitting `completed_at`.

**Final Critique 5 findings closure — 2026-08-26 (both foundational + six refining + three rhetorical folded; gated by Final Critique 6 below).** Implementation pass closing every routed Final Critique 5 finding, per the re-grounding campaign (shallow compositions after the atoms). **Caller signatures and the rejection taxonomy are UNCHANGED.** Foundational first: FC5-F1 → the no-op [Edit] defined by mirroring the constituent exactly — a normalized-equal edit appends nothing, captures no [Prior Description], leaves the derived state untouched, returns `ok`, cannot `storage-failure` (no append to fail), and is never an [Undo] target — with Invariant 1 carved to *state-changing* actions and the no-op named as the one accepted non-appending action; FC5-F2 → Invariant 7 restated as walking back through the states of the **current non-undone event sequence** (each undo exposes that sequence's replay minus its newest event), with both unreachable classes named as Redo's territory — the state each undo itself abandons (add → complete → undo leaves Done behind) and states behind events already undone once forward actions resume. Refining: Invariant 4 now carries the per-replay scoping the [Undo] wiring states (within a replay every Personal Todo invariant holds, temporal ones included; across replays the exposed timeline may deliberately rewind — designed undo behavior, not a constituent violation); the `snapshot`/`prior_description` replay-sufficiency claim replaced with the honest justification — replay never consumes either; both are audit-and-inspection conveniences readable via [Read History] — stated in Event schemas and the [Snapshot] card; Event Log's `invalid-payload` arm foreclosed by construction (five fixed machine-built schemas; a return of it is a composition defect surfaced as a deployment fault, never a caller rejection); the four forward wirings' "update the derived state" each cite Replay semantics as [Undo] does; API, UX, and GoF glossed at first use; the invariants section closes with its verification-surface sentence (records-alone by replay). Rhetorical: the `event_id`/`recorded_at` supply re-cited correctly (append-time assignment at the log's seam; the immutability invariant fixes them post-append); the [Nothing To Undo] card states the empty-log case plainly; Invariant 6's restored-field list gains `completed_at`. *Model impact:* `undo-history.tla` and its three buggy twins untouched — the closures are prose scoping, card, and gloss changes over the same replay semantics the model checks; the formal-layer vote is unchanged.

**Final Critique 6 — 2026-08-26: CLEAN (0 foundational); composition re-grounded.** The closing fresh-reader gate over the Final Critique 5 findings closure (AI-conducted, claude-fable-5, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). Both Final Critique 5 foundational closures held — the no-op [Edit] mirror and the restated Invariant 7 walk. The round returned **zero foundational**, seven refining, and two rhetorical findings; per the campaign stop rule (one fix pass, one gate, ground on clean or route), the composition re-grounds at `grounded on Final Critique 6 — 2026-08-26`, and the residue is recorded here as open routed findings — non-blocking under the 92%-good threshold (foundational at zero). *Refining (routed, open):* whether the recorded `description` is the raw caller input or Personal Todo's normalized (trim + NFC) form is unpinned, and replay applies the recorded form verbatim — the one-sentence pin (record the normalized form) protects the duplicate-active check under replay; the Replay-semantics validity claim omits the LIFO argument that makes it true (an event older than a surviving event can never enter the undone set without that event entering first, so every surviving event replays against exactly the state it was validated against — worth stating, since Invariant 4 leans on it); the `invalid-payload` foreclosure is real only under an unstated instance-configuration obligation (Event Log's arm is size-triggered, default 64 KB per instance — the owned instance's cap must be declared to accommodate the largest schema, a delete snapshot at maximum description length); the verification-surface sentence overreaches for Invariant 1's two unwitnessable clauses (no independent record of user actions exists to compare the log against — scope the sentence or name the checkable portion); the event `type` discriminator and its four wire values, returned verbatim as [Undone Event Type] values callers switch on, carry no cards or `Projects:` tokens against the Terms preamble's rule; the constituent specs' forward descriptions of this composition are stale in both directions (Event Log's Composition notes describe an Event Log + Reverse Index + Restore design; Personal Todo's describe delete-only events with administrator restoration — neither matches the five-event replay design; flagged for those atoms' next touch, a cross-pattern routing); I/O unglossed at first use. *Rhetorical (routed, open):* [New Description] is carded as an edit-event Field but used for the caller-supplied parameter in the no-op sentence (Personal Todo cards that datum as a Parameter projecting `newDescription`); the Event-schemas block lists the `event_id`/`recorded_at` envelope fields inside the payload braces, inviting a momentary wrong read the neighboring sentence resolves.
