...
formal: verified — chain-of-custody.tla + 2 twins; Invariant 7 not yet modelled, obligation open; 2026-06-11
last gate: 2026-08-26 — authentication-precedence gate, fresh reader — 1 foundational routed, 2 foundational and 8 refining/rhetorical corrected in-round, 7 refining and 5 rhetorical routed; Final Critique 7's 2 other foundational, 7 refining and 4 rhetorical also still open

open:
- 2026-08-26-a · foundational · Invariant 3 · describes the cascade as scrubbing the attestation's personal-data fields, where the substrate destroys only the attestation's `proof` and requires `actor_ref`, `action_ref`, `attested_at`, `attestation_id` to survive — and this composition's own `recovery_attested` fallback depends on the surviving actor → state the constituent's actual rule
- 2026-08-26-b · foundational · [Verify Custody] step 3b; `original_event_payloads` · keyed by `entry_id` and presenting one payload per `verify_record` call, where the substrate requires every payload in the covering seal's range in sequence order; under the interval cadence this spec recommends every entry fails with `seal-record-set-mismatch`, and intent records carry no `entry_id` so the map provably cannot key what the seal demands → surface the covering range through `read_record`, or make per-event cadence a wiring requirement; align the recommendation either way
- 2026-08-26-c · foundational · Configuration; Invariant 1; `recovery_attested`; verdict; check 1; partial-failure leg (c); failure-path example · `cascade_recovery` is defined as recovery-identity attestation in place of the custodian's, but leg (c) marks every compensating event and the example shows it on a custodian-attested retry, under which check 1 fails a conforming retry → pin the marker to the recovery-identity arm only; correct leg (c) and the example
...
- 2026-08-26-g · refining · recovery discipline, bind-to-existing arm; Invariant 4 · `cascade_recovery` cannot be set on that arm (no compensating event; entries immutable), falsifying the distinguishability claim there → state the arm's distinguishing evidence
...
- 2026-08-26-l · rhetorical · The load-bearing wiring decision · "in the same transactional boundary" reads as an atomicity claim Invariant 4 disclaims → rephrase
