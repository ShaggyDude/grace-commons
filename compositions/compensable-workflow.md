...
- `start_compensable workflow` with a definition whose *charge* step declares no compensation and is not marked `read-only` or `pivot` → `rejected(invalid-definition)`. The compensable workflow never starts; the composition refuses to promise all-or-compensated for an irreversible effect.
