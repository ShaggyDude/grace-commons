...
**Pass 1 — GRID structural (GRID — the nine-node completeness framework: Intent, System, Friction, Flow, Decision, Feedback, State, Behavior, Proof).** All nine MUSE (the v1.1 completeness framework GRID's nodes are drawn from) nodes resolved. Intent (the ordering problem; principal-binding as emergent property), System boundary (two atoms, one action, no cross-atom state), Friction (expired / revoked / not-known gate cases), Flow ([Check Permitted] two-step sequence), Decision (gate clears → query; gate fails → reject), Feedback (result tags are first-class; `denied` vs. `rejected` are structurally distinct), State (no cross-atom state; constituent atom state is ground truth — named as deliberate architectural choice), Behavior (four invariants), Proof (generation acceptance). Clean.
...
**Composition-state classification fold — 2026-08-26 (batched pre-convention tail, methodology debt #9; gated by Final Critique 7 below).** Convention retrofit for the composition-state rule adjudicated 2026-06-10, which postdates this composition's grounding. Change: the existing no-cross-atom-state declaration given the rule's vocabulary — **conforming, no stored composition state** — with the gate pinned as a per-call sequencing constraint over declared constituent surfaces, nothing stored, nothing to rebuild. The section already said this in substance; the fold cites the rule. **Caller signatures UNCHANGED**; no invariant number, signature, or rejection taxonomy changed. Gate: closed by Final Critique 7 below.

**Final Critique 7 — 2026-08-26: not clean (2 foundational; ROUTED, not folded — the tail round ends here).** The closing fresh-reader gate over the 2026-08-26 convention fold (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). Every constituent-contract claim verified verbatim (Session's `validate` outcome vocabulary and revoked-over-expired precedence included), Pass 2 clean (the gate correctly declines its own extraction — pure sequencing, no state machine, Gate 3 fails), and the denial-vs-rejection invariant, the validate/revoke TOCTOU honesty, and the named bypass non-conformance all credited as defended. It surfaced **two foundational findings** plus six refining and two rhetorical. Per the stop rule, all are recorded as **open routed findings**; the composition is downgraded to `partially resolved` until they close and a round returns zero foundational. The two foundational:

- *FC7-F1 — the `invalid-request` predicate is undefined — foundational (OPEN) →* "absent or malformed" is never defined for either opaque input, the composition carries no string policy (both constituents declare one), and the step-1 rejection sits in undeclared tension with Permissions' own empty-input→`denied` semantics — so conforming implementations split a security-distinct outcome boundary (the spec's own Invariant 3 says the classes must not be collapsed) between `invalid-request`, `denied`, and `session-invalid(not-known)` at their own discretion. Fix: a string-input policy paragraph (byte-exact discipline; absent/malformed = null, empty, whitespace-only, over-cap), the step-1 precedence over Permissions' empty-input semantics stated, and `invalid-request` owned as composition-introduced.
- *FC7-F2 — the caching edge case sanctions what Invariant 1 forbids — foundational (OPEN) →* Invariant 1 says an invalid session never reaches the permission check and the Standards section claims ASVS V3.3 by per-call re-validation, yet the caching edge case licenses cross-call validation caches (requiring only documentation) — under which a revoked session reaches `Permissions.permitted` with `validate` never consulted. Fix: pick a side — declare intra-request caching non-conforming (aligning invariant and ASVS claim), or scope Invariant 1 to point-in-time-of-validate with caching an explicit policy exception and the ASVS claim qualified.

*Refining/rhetorical (open, from Final Critique 7):* Invariant 4's "regardless of session validity" literally false for invalid sessions (fix the conditional); the Notes' "Permissions' own internal fail-safe result" attributing a Check-read-failure behavior the constituent never declares; "terminal" used against Session's careful lapse-is-not-terminal vocabulary; no example exercising `invalid-request` (adding one forces FC7-F1's answer); the Terms preamble misclassifying `invalid-request` as constituent-inherited when neither wired operation can produce it; "atom"/"regulated composition" unglossed in the Summary. Rhetorical — "PAP" never bound at first use; the AC-12 enforcement claim mildly overstated.

**Final Critique 7 findings closure — 2026-08-26 (both foundational + six refining + two rhetorical folded; gated by Final Critique 8 below).** Implementation pass closing every routed Final Critique 7 finding, per the re-grounding campaign (shallow compositions after the atoms). **Caller signatures and the rejection taxonomy are UNCHANGED.** Foundational first: FC7-F1 → a *Primitive policies* paragraph added to Composition logic defining the `invalid-request` predicate exactly (absent/malformed = null, empty, whitespace-only, or over the deployment-pinned length cap; accepted values handled byte-exact with no normalization, the constituents' own comparison discipline), declaring the rejection **composition-introduced** (neither wired operation can produce it), and stating the boundary's precedence over Permissions' empty-input→`denied` semantics — that constituent path is deliberately unreachable, keeping the three outcome classes security-distinct as Invariant 3's discipline extended to inputs; the Arguments, step 1, Terms preamble, and a new [Invalid Request] card + registry line all align, and a new *Malformed input* example exercises the arm (closing the no-example refining finding with it); FC7-F2 → the side is picked **for the invariant**: the caching edge case now declares cross-call validation caches non-conforming — Invariant 1's per-call `validate` and the ASVS V3.3 claim stand as written, the point-in-time meaning is bounded by the concurrent-revocation edge case, and the admissible optimization is named as making `validate` cheap, never skipping it. Refining: Invariant 4 conditioned on the gate clearing (the old "regardless of session validity" was literally false for invalid sessions); the Notes' invented "Permissions' internal fail-safe `denied`" attribution removed — the constituent declares no read-failure outcome, and the composition now says so honestly (an unreadable grant store is an availability fault with no conforming outcome, no invented pass-through); "terminal" replaced with Session's own vocabulary (invalid — expired or revoked) in the disputed-access scenario and Generation-acceptance check 1; the `invalid-request` example added (above); the Terms preamble's misclassification corrected (carded as composition-introduced, not constituent-inherited); "atom" and "regulated composition" glossed in the Summary. Rhetorical: PAP bound at first use in the Composition notes; the AC-12 claim rescoped — termination is Session's and Login's act, this composition contributes the access-time refusal. *Model impact:* `session-gated-authorization.als` untouched — the closures are boundary-predicate prose, card, and gloss changes outside the modeled gate ordering; harness standing unchanged.

**Final Critique 8 — 2026-08-26: CLEAN (0 foundational); composition re-grounded.** The closing fresh-reader gate over the Final Critique 7 findings closure (AI-conducted, claude-fable-5, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). Both Final Critique 7 foundational closures held — the Primitive-policies predicate and the caching non-conformance ruling. The round returned **zero foundational**, four refining, and three rhetorical findings; per the campaign stop rule (one fix pass, one gate, ground on clean or route), the composition re-grounds at `grounded on Final Critique 8 — 2026-08-26`, and the residue is recorded here as open routed findings — non-blocking under the 92%-good threshold (foundational at zero). *Refining (routed, open):* the composition's deployment-pinned cap is never related to the constituents' own per-string caps — Session's String input policy rejects over-length inputs as its own [Invalid Request], so a composition cap exceeding a constituent's can let a constituent outcome through that the step enumeration does not handle (the fix is one cap-alignment sentence: the boundary cap must not exceed either constituent's); the direct-access edge case leaves unresolved whether Invariants 1–2 are scoped to [Check Permitted] calls or are system-wide when a documented bypass is exposed; Generation-acceptance check 5's recorded-outcome vocabulary omits `invalid-request`, silently dropping a declared outcome class from the audit-record schema; checks 1–2 state absolute at-time-T guarantees without the concurrent-revocation point-in-time bound the edge case carries. *Rhetorical (routed, open):* "expired-by-revocation" in the Login note conflates the two vocabularies Invariant 3 keeps distinct; the not-known example's "already purged" cause names a deletion surface neither constituent declares; the shared Terms boilerplate's "one of four" enumerates five kinds.

**Showcase pass — 2026-06-29.** Representational-only annotation/legibility pass; no guarantee, invariant, number, formula, signature, or rejection taxonomy changed (the invariant count held at four). (a) **Four-kind `[Term]` annotation** applied across the body and a `## Terms` registry added after Edge cases. Because this is a **minimal, stateless gate composition** (one action, no cross-atom state), the registry is honestly small — 2 terms: 1 Operation, the single [Check Permitted] action; and 1 Member, the composition's own [Session Invalid] gate rejection. Nothing else is the composition's own: the emergent guarantees it owns (session-gates-authorization, principal binding) are structural Invariants, not data, and everything else is relayed from Session and Permissions. Every own-concept prose reference is linked. Survivors left backticked: the fenced projected-contract signature and the example blocks; the qualified constituent calls (`Session.validate` / `Session.revoke`, `Permissions.permitted`); the relayed outcomes (`permitted`, `denied`) and Session's `invalid(...)` reasons (`expired` / `revoked` / `not-known`); the reason-parameterized `rejected(session-invalid(...))` wire forms; and the generic `invalid-request`. Constituent atom names remain the existing full links to `../atoms/*`. One representational tidy: the `### check_permitted` action heading was backticked to `### \`check_permitted\`` (removing a bare snake-case heading and giving the card a distinct anchor). (b) **Summary** — this file carried no `## Summary`, only a descriptive top blockquote; the blockquote was **promoted to a `## Summary` section** (moved above Intent, split one-idea-per-paragraph, lossless) so the file matches the standard, and its subject was named. (c) **Lineage collapsed** into a `<details markdown="block">` block (the `### Round 1`–`### Round 6` subsections collapse inside). (d) **prose cut #5 — skipped (with reason):** the composition owns no state machine — it is a stateless sequencing gate over Session's and Permissions' own state; the session lifecycle (Valid / Expired / Revoked) is Session's. Re-verified, not re-grounded: Status stays at `grounded on Final Critique 6 — 2026-05-23`. Gates: lint clean (O-term resolver — every marker resolves and every card is used); term-adapter derives cleanly (2 terms); four composition-level invariants preserved; the `.als` model untouched — harness re-run green (Alloy `session-gated-authorization.als`).
