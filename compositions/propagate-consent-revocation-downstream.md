...
5. Call `Consent.revoke(consent_id, revoked_by=actor_ref, reason)` → `revoked` (or map: `not-known` → `rejected(recording-failure)` — an internal-consistency anomaly, since step 2 resolved the id in this composition's own map but Consent does not know it; `already-revoked` → `rejected(already-revoked)`; `already-expired` → `rejected(already-expired)`; `invalid-request` → `rejected(invalid-request)`; `storage-failure` → `rejected(recording-failure)`. Stop on any — no propagation event is recorded if the revoke did not occur).
