...
3. Stamp [Fired At] from the seam-injected `now` (*Logic confinement*) and call `Subscription.subscribers_for(event_scope)`. The stamp is taken immediately before the query: it is the instant the subscriber set is fixed, and it is what the caller's audit entry pins (Generation acceptance check 1).
...
A caller who receives a non-empty [Failed] list and wishes to retry has two options. First: retry each `subscriber_ref` in the [Failed] list directly, **reconciling before creating**: a [Failed] entry may be indeterminate (action wiring step 4), so the caller first reads `Notification.pending_for(subscriber_ref)` and, if a Pending record carrying this fanout's payload is already there, treats the delivery as recorded and does not create; only otherwise does it call `Notification.create(subscriber_ref, payload)`. A caller that composes Duplicate Prevention keyed on `(fanout_id, subscriber_ref)` around the retry gets the same guarantee without the scan. A direct `create` with neither is the one path that can give a subscriber two records from one fanout, and it is not a conforming retry. This retries exactly the failed creates without re-querying the subscriber set. Second: call [Fanout] again — this re-queries `subscribers_for`, which may return a different subscriber set if subscriptions have changed in the interim. The first option is correct when the caller needs to deliver to exactly the original fanout's subscriber set; the second is correct when delivering to the current Active set is the right behavior. Callers who need at-most-once fanout semantics across retries should compose Duplicate Prevention to guard the [Fanout] call itself; see Edge cases.
...
The seam-injected clock reading the composition took at step 3, immediately before `subscribers_for` — the instant the subscriber set was fixed. Returned so the caller's Event Log entry can pin the instant the composition observed; the caller never reproduces it. It is the composition's one clock use (*Logic confinement*) and is a different seam's reading from each record's `created_at`.

Kind:      Field
Field of:  the fanout result
Role:      the instant the subscriber set was fixed
Projects:  fired_at
