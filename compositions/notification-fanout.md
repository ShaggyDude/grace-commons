...
The fan-out decomposition model was formalized in [`execution-contract.md`](../execution-contract.md) before drafting: fan-out is a decomposition boundary, not a single transition; the composition is a stateless interpreter of a directed invocation graph; no rollback guarantee exists across independent creates; partial failure is per-recipient and named in the result, not an abort condition. The draft follows this model directly.
...
- **Invariant-count cross-references remain structurally fragile.** The mechanical fix (drop counts, say "all Subscription invariants" without a number) loses precision but eliminates staleness risk. pressure-testing.md already names this hazard; whether to do the library-wide rewrite is an authorial call. Encountered twice now (Shared Todo's nine-vs-ten on Assignment, this composition's eight-vs-nine on Subscription); the third encounter probably forces the convention.
- **Closed-action vs. open-audit tension.** The two constituent atoms' refinement rounds resolved this per-pattern by adding an explicit audit-surface preamble to each Generation acceptance section. A canonical statement of the distinction belongs in `pressure-testing.md` or `contributing.md` so future patterns inherit the convention rather than re-derive it.
...
**Composition-state classification fold — 2026-08-26 (batched pre-convention tail, methodology debt #9; gated by Final Critique 5 below).** Convention retrofit for the composition-state rule adjudicated 2026-06-10, which postdates this composition's grounding. Change: the existing *None* declaration given the rule's vocabulary — **conforming, no stored composition state** — and the record-that-a-fanout-ran routing to a composed Event Log named as the Contract's record-coordination rule applied, which the section already did in substance. **Caller signatures UNCHANGED**; no invariant number, signature, or rejection taxonomy changed. Gate: closed by Final Critique 5 below.

**Final Critique 5 — 2026-08-26: not clean (1 foundational; ROUTED, not folded — the tail round ends here).** The closing fresh-reader gate over the 2026-08-26 convention fold (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). The gate found the composition "unusually well defended" — crash-mid-execution, double-invocation, query-to-create staleness, the empty-set non-error, the all-or-nothing objection, the invalid-request structural-inconsistency diagnostic, and the audit-binding gap all anticipated — and verified the constituent signatures, invariant counts, and ordering delegations verbatim. It surfaced **one foundational finding** — load-bearing, pervading the wiring decision, retry semantics, an example, a scenario, and Terms — plus six refining and two rhetorical. Per the stop rule, all are recorded as **open routed findings**; the composition is downgraded to `partially resolved` until they close and a round returns zero foundational. The foundational:

- *FC5-F1 — `storage-failure` attributed to `Notification.create`, which the constituent does not declare — foundational (OPEN) →* Notification's `create` rejects only `invalid-request` (its transition table, Decision points, Feedback, and Terms all agree; no storage-failure card exists) — the composition's primary failure mode, the entire reason the [Failed] list and continue-through-failures decision exist, is wired to a token the constituent does not expose (the symmetry with Subscription's write-side `storage-failure` was assumed, not real). Fix, two admissible forms: (a) amend Notification to declare a `storage-failure` arm on `create` with fail-closed semantics, mirroring Subscription's writes — the true minimal fix, a constituent touch the triager sequences; or (b) reword the composition to own the classification (any non-`notification_id` outcome at the write step lands the subscriber in [Failed]), removing `storage-failure` from the inherited-rejections list, paralleling the composition-owned `subscribers-unavailable`.

*Refining/rhetorical (open, from Final Critique 5):* Generation-acceptance check 1(d)'s exact-equality demand missing the clock caveat its own adversarial section carries, with `fired_at` never pinned to the query moment; Invariant 8's system-lifetime [Fanout Id] uniqueness resting on `entropy.generate()` with no declared entropy floor (both constituents declare one); the Summary's unconditional exactly-one claim missing the crash-path disclaim the edge case carries; step 1's validation justified by mirroring `subscribe`, an action the composition never calls (the check is legitimately its own policy); the invariants preamble contradicted by Invariants 6–7 (preserved constituent invariants); "Q surface" undefined. Rhetorical — "the atom" as a self-reference in Primitive policies; GoF, AMQP, SMTP, APNs/FCM unglossed in Standards references.

**Final Critique 5 findings closure — 2026-08-26 (the one foundational + six refining + two rhetorical folded; gated by Final Critique 6 below).** Implementation pass closing every routed Final Critique 5 finding, per the re-grounding campaign (shallow compositions after the atoms). FC5-F1 was closed in its **form (b) — the composition owns the classification**; form (a), a `storage-failure` arm added to Notification's `create`, remains available as a future constituent touch but was deliberately not taken mid-campaign (it would reopen a grounded atom for a fix the boundary can own outright). The reworking: action wiring step 4 now enumerates the constituent's actual contract (`notification_id | rejected(invalid-request)`) and classifies **any other outcome** — store unavailable, timeout, no conforming constituent outcome — at this boundary as the subscriber's fanout failure, the write-side counterpart of the composition-owned [Subscribers Unavailable]; the `storage-failure` token is gone from the wiring, the continues-through-failures rule, the retry semantics, the partial-failure example (which now shows the write failing with no conforming outcome rather than a token the constituent never declares), the disputed-notification scenario, the transient-vs-structural edge case, the Terms preamble's inherited-rejections list, and the [Failed] card. Refining: Generation-acceptance check 1's entry shape pins `fired_at` to the moment of the `subscribers_for` query, and check 1(d)'s equality now carries Subscription Invariant 9's clock caveat with the boundary-adjacent-mismatch resolution rule; Invariant 8's uniqueness claim carries a declared entropy floor (≥128 bits per id or an equivalently coordinated generator, matching the constituents' floors), cross-referenced from wiring step 2; the Summary's exactly-one claim is scoped to invocations that run to completion, naming the crash-mid-fanout case the idempotency edge case owns; step 1's validation is restated as the composition's own boundary policy — consistent with, not inherited from, the constituents (the old text justified it by mirroring `subscribe`, an action this composition never calls); the invariants preamble now distinguishes emergent Invariants 1–5/8 from preservation Invariants 6–7; Invariant 7's undefined "Q surface" replaced with Subscription's declared query surface (`subscribers_for`). Rhetorical: "the atom" self-reference in Primitive policies corrected to the composition; GoF, AMQP, SMTP, and APNs/FCM glossed in Standards references. *Model impact:* none — this composition is English-only (formal-layer vote NO).

**Final Critique 6 — 2026-08-26: not clean (1 foundational; ROUTED, not folded — closure round ends here).** The closing fresh-reader gate over the Final Critique 5 findings closure (AI-conducted, claude-fable-5, Happy-Torvalds-X2, fresh-reader discipline throughout — pass question sets, the spec body, and both constituent specs for structural checking only; no Lineage, no prior findings). The FC5-F1 boundary-owned classification held cleanly — the gate treated it as a declared composition-owned judgment, not a constituent-contract violation. It surfaced **one new foundational finding** — introduced by the FC5 closure's own `fired_at` pin — plus nine refining and three rhetorical. Per the stop rule (one fix pass, one gate, then ground or route), all are recorded as **open routed findings**; the composition holds at `partially resolved` until they close and a round returns zero foundational. The foundational:

- *FC6-F1 — the query-moment `fired_at` pin is unimplementable from the declared contract — foundational (OPEN) →* Generation-acceptance check 1 requires the Event Log entry's `fired_at` be stamped at the moment of the `subscribers_for` query, but [Fanout] returns only `{fanout_id, created, failed}` and log-writing is assigned to the caller, who cannot observe the query instant — the audit-binding story is unimplementable as declared (an error introduced by the FC5 closure edit). Fix, two admissible forms: (a) extend the [Fanout] result shape with an additive `fired_at` field carrying the query-moment reading (seam-injected clock, per Logic Confinement), so the caller logs what the composition observed; or (b) relax the pin to caller-observable invocation time with an honest audit-binding caveat bounding the gap.

*Refining/rhetorical (open, from Final Critique 6):* Invariant 8's "same floor each constituent declares" is false for Notification (no entropy floor declared there; the composition's own 128-bit floor stands independently — restate it as its own); wiring step 1's "nothing is inherited here" vs the Terms preamble's "one inherited rejection" disagree on `invalid-request` ownership; the Generation-acceptance preamble's stores-only evidence base contradicts checks 1–2's Event-Log-required posture (state the bare-deployment acceptability honestly); step 4's "structural inconsistency" is an ordinary reachable path with nameable causes (Subscription admits whitespace-only refs and no length cap; Notification rejects both) — name them; "standards inheritance in full" omits Subscription's XMPP PubSub (XEP-0060); W3C unglossed twice; the Summary's "idempotency" unglossed at Tier-1; [Fanout Id]'s inline `entropy.generate()` diverges from the constituents' seam-injection discipline with no stated rationale (inject the id at the seam, or defend the divergence); the externally-clearable disclosure list omits Permissions and Audit Trail, both named in Edge cases. Rhetorical — the Summary's "fixed at the moment the event fires" vs the normative query-execution instant; Invariant 6's "over each created record" phrasing for store-wide constituent invariants; the compliance walkthrough's gloss of [Expired] as "no delivery attempt" narrowing the constituent's window-lapse semantics.
