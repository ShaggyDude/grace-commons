...
**Formal-layer vote — 2026-06-03: NO.** Emergent invariants (fanout coverage, payload consistency, at-most-one per subscriber) are single-invocation structural coverage properties — no ordering, concurrency, or gate-before-action claim. Grounds English-only (minimum-formalism). Vote per [`PRESSURE_TESTING.md`](../PRESSURE_TESTING.md) §Formal models — The formal-layer vote.
