...
- *FC6-F1 — the query-moment `fired_at` pin is unimplementable from the declared contract — foundational (**CLOSED 2026-08-27** — taken in **form (a)**: [Fanout]'s result shape gains an additive `fired_at` carrying the seam-injected query-moment reading, so the caller logs what the composition observed instead of a time it cannot see. Form (b), relaxing the pin to caller-observable invocation time, was available and not taken: it would have preserved the entry shape by weakening the claim, and the claim is the audit binding — the same preference the atomicity class settled on, that where the cost is bounded and additive, make the claim true rather than qualify it. No caller breaks; a caller ignoring the field degrades to the clock caveat rather than to a false pin.) →* Generation-acceptance check 1 requires the Event Log entry's `fired_at` be stamped at the moment of the `subscribers_for` query, but [Fanout] returns only `{fanout_id, created, failed}` and log-writing is assigned to the caller, who cannot observe the query instant — the audit-binding story is unimplementable as declared (an error introduced by the FC5 closure edit). Fix, two admissible forms: (a) extend the [Fanout] result shape with an additive `fired_at` field carrying the query-moment reading (seam-injected clock, per Logic Confinement), so the caller logs what the composition observed; or (b) relax the pin to caller-observable invocation time with an honest audit-binding caveat bounding the gap.
