...
**Status note.** With the refinement round complete and findings closed, the spec is positioned for a clean re-run before declaring `grounded`. Per the methodology's iteration rule ("the loop runs until a complete pass surfaces no new findings"), one more pass over the modified document is the honest path; status is held at `partially resolved` pending that confirmation.
