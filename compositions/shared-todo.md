...
**Final Critique 5 findings closure — 2026-08-26 (all three foundational + seven refining + three rhetorical folded; gated by Final Critique 6 below).** Implementation pass closing every routed Final Critique 5 finding, per the re-grounding campaign (deep compositions after the shallow). **Two additive contract changes, no parameter removals:** [Assign Task] gains a `not-known` arm, and the two derived read queries gain the gating `actor_ref` plus explicit rejection arms their own declared wiring already required (the reads were stated as `tasks:view`-gated while their signatures carried no actor and no arm — the closure makes the declared behavior expressible). Foundational first: FC5-F1 → [Assign Task] picks up Assignment's explicitly-delegated referential-integrity check as its new step 2 — the task must exist in Pending **or** Done (assigning Done is deliberately admitted as the completion-attribution pattern; a never-existing or deleted id returns `not-known` and never reaches `Assignment.assign`) — making the cascade rule's *no Active assignment references a deleted task* a standing invariant rather than a delete-time-only one; FC5-F2 → a *Grant administration is out of scope* edge case declares the `grant`/`revoke` surface a governance non-goal of the canonical composition, with [Attributed Permissions Admin](./attributed-permissions-admin.md) named for regulated deployments over the same instance; FC5-F3 → a *`actor_ref` ↔ authenticated-caller binding* edge case names the binding a deployment-seam assumption with the suppliers named ([Login](./login.md) + [Session-Gated Authorization](./session-gated-authorization.md) providing the session-extracted principal, or [Authenticated Actor](./authenticated-actor.md) for the credential-bound form), the guarantees stated over presented `actor_ref` values. Refining: SLA glossed; the read-query denial semantics pinned deterministically to [Permission Denied] (the empty-result-or-rejection deployment choice removed; an empty list is an answer about the task set, not an access answer) with both query contracts updated; [Responsible Actor] given its `not-known` arm, making the existence side of the join observable (`unassigned` = exists-without-assignment, `not-known` = never held); a *Partial [Delete Task]* edge case names the recall-committed-then-delete-failed state (over-recall on the safe side of one-way Invariant 3, visible via `unassigned`, remedied operationally; the recall-first ordering defended as what keeps the failure safe); the clinical example's attribution claim tempered — Assignment records the responsibility chain, not the acting caller, and invocation-level attribution is Audit Trail's; the support-queue roster now grants tier-2 the scopes the example exercises; a *Single-instance consequences* edge case states global description uniqueness and the `duplicate-active` existence oracle with the partitioning remedy. Rhetorical: the Composes gloss carries Personal Todo's delete-from-either-state; the visibility predicate restated per-actor (not per-task) in the canonical deployment; the sprint manager's roster gains the `tasks:view` grant the walkthrough exercises. *Model impact:* `shared-todo.tla` and its buggy twin untouched — the additive existence check and query arms sit outside the modeled permission-gate/cascade behavior; the formal-layer vote is unchanged.

**Final Critique 6 — 2026-08-26: CLEAN (0 foundational); composition re-grounded.** The closing fresh-reader gate over the Final Critique 5 findings closure (AI-conducted, claude-fable-5, fresh-reader discipline throughout — pass question sets, the spec body, and the three constituent specs for structural checking only; no Lineage, no prior findings). All three Final Critique 5 foundational closures held — the assign-side existence check, the grant-administration non-goal, the authentication seam. The round returned **zero foundational**, seven refining, and four rhetorical findings; per the campaign stop rule (one fix pass, one gate, ground on clean or route), the composition re-grounds at `grounded on Final Critique 6 — 2026-08-26`, and the residue is recorded here as open routed findings — non-blocking under the 92%-good threshold (foundational at zero). *Refining (routed, open):* the wiring paragraph names a third read query ("task detail by id") with no contract, shape, arms, or card — specify it or strike it; [Delete Task] step 2 is silent on `recall`'s `not-known`/`not-active` arms, whose unreachability depends on whether the serialization unit is the composed action or the constituent call (the Concurrency edge case never pins which); the *Partial [Delete Task]* edge case's concurrent-delete branch describes a state no interleaving produces (the winning delete removed the task; the loser's recall never committed) though the safe-side claim stands; the [Responsible Actor] card's "a task the store never held" disagrees with the join semantics under which a deleted task also answers `not-known`; the assignee-side referential question (`assignee_ref` against a real actor) is neither picked up nor named a non-goal, asymmetric with the two seams the closure did address; the composition-introduced `not-known` assign guard is exercised by no example while permission denial is exercised twice; OAuth2 (and the clinical q4h) unglossed inside quoted example data against the mechanical first-use rule. *Rhetorical (routed, open):* the Composition-state "none otherwise" invites the empty-list reading the wiring forbids two lines later; Invariant 1's action list can be read as demanding a `tasks:recall` check for the cascade's internal recall; the Done-assignment rationale borrows the completion-attribution edge case, which describes an assignment held at completion rather than created after it; the single-instance uniqueness claim says "Pending" where Personal Todo's active set spans Pending ∪ Done.
