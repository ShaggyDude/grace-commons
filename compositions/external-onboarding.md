...
1. Validate inputs. If `inviter_ref`, `context`, or (if supplied) `ttl` fails validation, or the constructed event data of either record below exceeds the wired Audit Trail instance's payload budget → `rejected(invalid-request)`. Stop. (The size check at this layer is what makes the record steps' `invalid-request` arm unreachable for composition-built payloads; observing it there is a deployment payload-budget fault.)
2. Call `Audit Trail.record_action(action_ref="invitation.initiate-attempt", actor_ref=inviter_ref, credential=actor_credential, data={invitee_ref, context, ttl})` → `event_id | rejected(invalid-request | invalid-credential | recording-failure)` — the credential gate.
   ...
   - `invalid-request` → unreachable per step 1; a deployment fault if observed, surfaced as `rejected(storage-failure)`.
...
1. Validate inputs. Any missing or malformed required field, or constructed event data over the wired instance's payload budget → `rejected(invalid-request)`. Stop. (The size check forecloses the record steps' `invalid-request` arm for composition-built payloads; observing it below is a deployment payload-budget fault.)
...
1. Validate inputs (including the constructed event data within the payload budget) → `rejected(invalid-request)` if invalid. Stop.
...
1. Validate inputs → `rejected(invalid-request)` if `revoked_by_ref`, `reason`, or `actor_credential` is absent or malformed, or the constructed event data exceeds the payload budget. Stop.
