...
- **Structural-relation invariants — canonical templates** *(added 2026-06-11; the guided tool's design review surfaced that no canonical home existed)*. When a composition declares a relation across its constituents (cardinality one-to-one / one-to-many / many-to-many; each side mandatory or optional — the declaration [`pressure-testing.md`](./pressure-testing.md) Pass 3 §Relations requires), its required invariants take these forms. *Referential integrity:* every reference resolves to an existing record of the named constituent. *Orphan-freedom* (only where a side is mandatory), stated conditionally **at quiescence** — as safety (no action of this composition leaves a mandatory side unbound) plus liveness (a binding orphaned by partial failure is surfaced and reconciled within the declared compensation window), modulo declared compensation and the retention horizon — never as a static "always both sides." *Inverse consistency:* the inverse direction agrees with the forward references, classified per [`execution-contract.md`](./execution-contract.md) §Composition state (derived index by default). *Acyclicity / well-foundedness* (only where the relation is **self-referential** — a constituent related to its own kind, directly or transitively: a containment hierarchy, a bill-of-materials, a category or org tree, an observation group; the one relation shape in which a cycle can arise at all): no record is its own ancestor under the relation's transitive closure, so the closure is well-founded and finite (a forest where the relation is single-parent, a DAG where it is multi-parent). The transitive closure itself — the flattened / exploded view — is a *derived index* per [`execution-contract.md`](./execution-contract.md) §Composition state (rebuildable, not owned state); where edges carry quantities, the rollup over the closure sums diamond-correctly. *(Added 2026-06-13. The concept-recovery exercise found this invariant recurring across six systems — BOM, category / location / role hierarchies, observation groups, FHIR `hasMember` — and routed it here as the fourth template rather than to a new atom: it owns no state of its own, only the integrity of a self-referential relation a host concept already declares. The general-structural-atom case — the Workflow / State Machine precedent — was weighed and declined: Workflow owns its state, an acyclic-containment relation does not. See [`working-ideas/acyclic-recursive-composition-plan.md`](./working-ideas/acyclic-recursive-composition-plan.md) and [`roadmap.md`](./roadmap.md) §Concept-recovery atom backlog.)* Owned here so tooling that auto-inserts them cites one home rather than minting phrasing; the governing semantics remain the methodology's Relations line.
