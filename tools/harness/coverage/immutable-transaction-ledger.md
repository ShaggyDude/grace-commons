...
> C6-2's GAP row: the prior model verified the atomic idealization of Invariant 1 and
...
| Invariant 1 — binding bijection, **safety arm, compensated path** ("or the failure is compensated": orphan reachable → surfaced → retried → bijection restored; no *unsurfaced* orphan at any reachable state) | **yes** | **covered** *(was GAP — closed 2026-06-10, finding C6-2)* | `FailPartial(d)` makes the orphan a real reachable inter-action state; `RetryAudit(d)` is the compensation; asserted by `Inv1_SafetyBijection` + `Inv1_NoUnsurfacedOrphan` across every interleaving of the three actions |
...
- GAP rows: **none** — the compensated arm's GAP (finding C6-2) is closed by this revision.
