...
**Round structure (baseline + Final Critique).** The 3×3 baseline is three rounds of the three passes — Round 1 runs Pass 1 / Pass 2 / Pass 3; Round 2 runs Pass 1 / Pass 2 / Pass 3; Round 3 runs Pass 1 / Pass 2 / Pass 3. Pass numbering resets per round so "Pass 3" always means the Linus adversarial pass, never the third pass-of-nine. After the baseline, Final Critique runs as an AI-conducted Super Torvalds round starting at Round 4; if Final Critique 4 closes clean, the pattern is `grounded on Final Critique 4`. If Final Critique 4 surfaces findings, a refinement round runs and the pattern grounds on Final Critique 5, then 6, and so on until the 95%-good threshold clears (foundational findings reach zero; refining and rhetorical findings may remain). The naming convention is `grounded on Final Critique N` where N is the round at which the Final Critique passes — this is the canonical grounding marker. Older patterns that used cumulative Pass 1–9 numbering (Defensible Retention is the example) carry a retro-mapping note explaining the relabel; the unified convention is per-round Pass 1/2/3 + Final Critique. This convention should land in [`PRESSURE_TESTING.md`](./PRESSURE_TESTING.md) as canonical methodology when that file is next refreshed; until then, this paragraph is the standing reference.
